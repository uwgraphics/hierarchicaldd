...
public:
    ...
    void Write_Interface_Map(std::string directory){
        std::ofstream file(directory+"/interface_map",std::ios::out|std::ios::binary);
        size_t n=interface_map.size();
        file.write(reinterpret_cast<const char*>(&n),sizeof(n));
        file.write(reinterpret_cast<const char*>(&interface_map[0]),n*sizeof(T_INDEX));
        file.close();
    }
    void Read_Interface_Map(std::string directory){
        std::ifstream file(directory+"/interface_map",std::ios::in|std::ios::binary);
        size_t n;
        file.read(reinterpret_cast<char*>(&n),sizeof(n));
        interface_map.resize(n);
        file.read(reinterpret_cast<char*>(&interface_map[0]),n*sizeof(T_INDEX));
        file.close();
    }
