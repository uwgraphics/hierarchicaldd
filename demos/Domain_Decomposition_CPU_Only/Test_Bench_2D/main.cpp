...
int main(int argc,char *argv[])
{
    ...
    parse_args.Add_Vector_2D_Argument("-size",VECTOR<double,2>(257,257),"n n","Size of 2D grid");
    ...
    parse_args.Add_Vector_2D_Argument("-origin",VECTOR<double,2>(0,0),"n n","Origin of The domain");
    parse_args.Add_Vector_2D_Argument("-sub_size",VECTOR<double,2>(64,64),"n n","Size of the sub-domains");
    ...
    for(int v=1;v<=d;v++) subdomain_size(v)=static_cast<unsigned int>(parse_args.Get_Vector_2D_Value("-sub_size")(v));
    LOG::cout<<"Size of subdomain        : "<<subdomain_size<<std::endl;

    T_INDEX origin;
    for(int v=1;v<=d;v++) origin(v)=static_cast<unsigned int>(parse_args.Get_Vector_2D_Value("-origin")(v));
    ...
    if(mode=="no_convertion"){
        ...
        PHYSBAM_FATAL_ERROR("Conversion is not supported in 2D!");
    }
    LOG::Finish_Logging();
    return 0;
}
