...
public:
    ...
    ~SPGrid_Domain_Decomposition_Solver(){
        ...
        for(int i=0;i<interface_matrix_generators_second_level.size();++i)
            delete interface_matrix_generators_second_level[i];};
    //#####################################################################
    ...
    //#####################################################################
    void Initialize_Preconditioner(int v_cycle_levels,T_INDEX size,T_INDEX subdomain_size,int sigma_levels,int nVcycles,int interior_smoothing,int boundary_smoothing,int sigma_smoothing,int n_sigma_mg,bool multilevel=false){
        ...
        int n_subdomains=splitter.Split(interface_matrix_generator,v_cycle_levels,sigma_levels,number_of_threads,
                                        &interface_matrix_generators_second_level,multilevel);

        T_INDEX sub_subdomain_size=subdomain_size;
        if(multilevel) for(int axis=1;axis<=d;++axis) 
          {PHYSBAM_ASSERT(sub_subdomain_size(axis)%2==0);sub_subdomain_size(axis)/=2;}

        LOG::cout<<"subsize: "<<sub_subdomain_size<<std::endl;
        if(number_of_threads)
            SPGrid_Computations::Threading_Helper<T_STRUCT_4,d>(*(flags_field.allocator),spgrid_gemini.Set().Get_Blocks()).Run_Parallel(SPGrid_Computations::Domain_Divider<T_STRUCT_4,d>(&T_STRUCT_4::flags,spgrid_gemini.Set(),size,T_INDEX(),sub_subdomain_size),number_of_threads);
        else
        ...
        dd_preconditioner = new SPGrid_Domain_Decomposition_Preconditioner<T,T_STRUCT_4,d,T_offset_ptr>(*(flags_field.allocator),spgrid_gemini.Set(),
                                                                                                        ...
                                                                                                        subdomain_size,size,flags_field.field,
                                                                                                        n_subdomains,sigma_levels,
                                                                                                        nVcycles,interior_smoothing,boundary_smoothing,
                                                                                                        sigma_smoothing,n_sigma_mg,number_of_threads,
                                                                                                        &interface_matrix_generators_second_level,multilevel);
        dd_preconditioner->Initialize(true,false,false);        

        dd_preconditioner->v_cycle_topology=new SPGrid_Computations::V_Cycle_Topology<T_STRUCT_4,T,d>(*(flags_field.allocator),spgrid_gemini.Set(),size,
                                                                                                      v_cycle_levels,&T_STRUCT_4::flags);
        dd_preconditioner->v_cycle_topology->Initialize(number_of_threads);
        dd_preconditioner->v_cycle_topology->Flaging(number_of_threads);
    }
    //#####################################################################
    // Function Solve
    //#####################################################################
    void Solve(){
        ...
        PhysBAM::CONJUGATE_GRADIENT_DUMPING<T,d> cg(stream_type,"output");
#endif
