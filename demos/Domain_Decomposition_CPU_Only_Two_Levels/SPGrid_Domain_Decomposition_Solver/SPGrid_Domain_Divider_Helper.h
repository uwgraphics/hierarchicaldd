...
public:
    ...
    Domain_Divider(unsigned T_STRUCT::* flags_field_input,SPGrid_Set<Flag_Array_Type>& set_input,T_INDEX domain_size_input,T_INDEX origin_input,T_INDEX sub_domain_size_input,
                   bool clear_flag_input=false)
        :flags_field(flags_field_input),set(set_input),origin(origin_input),sub_domain_size(sub_domain_size_input),domain_size(domain_size_input),clear_flag(clear_flag_input)
    {}
    Domain_Divider(SPGrid_Allocator<T_STRUCT,d>& allocator,const std::pair<const unsigned long*,unsigned>& blocks,unsigned T_STRUCT::* flags_field_input,SPGrid_Set<Flag_Array_Type>& set_input,
                   T_INDEX domain_size_input,T_INDEX origin_input,T_INDEX sub_domain_size_input,bool clear_flag_input=false)
        :flags_field(flags_field_input),set(set_input),origin(origin_input),sub_domain_size(sub_domain_size_input),domain_size(domain_size_input),clear_flag(clear_flag_input)
        ...
        for(SPGrid_Block_Iterator<T_MASK> iterator(blocks);iterator.Valid();iterator.Next_Block()){
            ...
            for(RANGE_ITERATOR<d> iterator(RANGE<T_INDEX>(base_index,base_index+allocator.Block_Size().template Cast<T_INDEX>()-1));
                ...
                if(flag&SPGrid_Solver_Cell_Type_Active){
                    ...
                    if(is_interface){
                        if(clear_flag){
                            flag=SPGrid_Solver_Cell_Type_Dirichlet;
                        }else{
                        ...
                        T_INDEX subdomain_index=index/sub_domain_size;
                        int id=subdomain_index(1);
                        for(int axis=2;axis<=d;++axis) id=id*number_of_subdomains(axis)+subdomain_index(axis);
                        /*int tmp=id;
                        for(int axis=d;axis>=1;--axis){
                            if(tmp%number_of_subdomains(axis)!=subdomain_index(axis))
                                LOG::cout<<tmp<<","<<number_of_subdomains<<","<<subdomain_index<<","<<axis<<std::endl;
                            PHYSBAM_ASSERT(tmp%number_of_subdomains(axis)==subdomain_index(axis));
                            tmp/=number_of_subdomains(axis);} 
                            PHYSBAM_ASSERT(id==id&SPGrid_Solver_PartitionID_Mask);*/
                        //We want the id start from 1. If it is an interface or inactive cell it will have id flag 0;
                        flag|=(id+1);}}}}
    }     
};
//#####################################################################
