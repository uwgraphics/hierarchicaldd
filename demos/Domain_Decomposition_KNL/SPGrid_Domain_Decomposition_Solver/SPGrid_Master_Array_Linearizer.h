...
namespace SPGrid{
    ...
    public:
        ...
        T_offset_ptr* prolongation_fine_blocks_aligned_buffer;
        T_offset_ptr* prolongation_coarse_blocks_aligned_buffer;
        T_offset_ptr* restriction_fine_blocks_aligned_buffer;
        ...
        T_offset_ptr interface_data_buffer_size;
        ...
        SPGrid_Master_Array_Linearizer()
            :b(NULL),b_x_minus(NULL),b_x_plus(NULL),b_y_minus(NULL),
             ...
             prolongation_fine_blocks_aligned_buffer(NULL),
             prolongation_coarse_blocks_aligned_buffer(NULL),
             restriction_fine_blocks_aligned_buffer(NULL)
        {
            if(posix_memalign((void**)&restriction_fine_blocks_aligned_buffer,page_size,page_size))abort();
            if(posix_memalign((void**)&prolongation_fine_blocks_aligned_buffer,page_size,page_size))abort();
            if(posix_memalign((void**)&prolongation_coarse_blocks_aligned_buffer,page_size,page_size))abort();
        }
        ...
        void Initialize_Interface(const std::pair<const unsigned long*,unsigned>& blocks){
            ...
            for(int i = 0;i < number_of_interface_blocks;++i){
                ...
                offsets_map_interface.Insert(blocks.first[i],((unsigned long)(i))*page_size);
        ...
        void PopulateRestrictionOffsets(const std::pair<const unsigned long*,unsigned>& fine_blocks,const std::pair<const unsigned long*,unsigned>& coarse_blocks,
            ...
            //if(restriction_fine_blocks_aligned_buffer) free(restriction_fine_blocks_aligned_buffer);
            ...
            if(posix_memalign((void**)&restriction_fine_blocks_aligned_buffer,page_size,
                              (coarse_n_blocks*27*sizeof(T_offset_ptr)/page_size+1)*page_size)) 
                abort();
            memcpy(restriction_fine_blocks_aligned_buffer,&(restriction_fine_blocks[0](0)),coarse_n_blocks*27*sizeof(T_offset_ptr));
        }
        void PopulateProlongationOffsets(const std::pair<const unsigned long*,unsigned>& fine_blocks,const std::pair<const unsigned long*,unsigned>& coarse_blocks,
            ...
            if(fine_blocks.second==0&&coarse_blocks.second==0) return;
            //if(prolongation_fine_blocks_aligned_buffer)free(prolongation_fine_blocks_aligned_buffer);
            //if(prolongation_coarse_blocks_aligned_buffer)free(prolongation_coarse_blocks_aligned_buffer);
            ...
            int n_meta_blocks = prolongation_fine_blocks.size();
            if(posix_memalign((void**)&prolongation_fine_blocks_aligned_buffer,page_size,
                              (n_meta_blocks*8*sizeof(T_offset_ptr)/page_size+1)*page_size)) 
                abort();
            memcpy(prolongation_fine_blocks_aligned_buffer,&(prolongation_fine_blocks[0](0)),n_meta_blocks*8*sizeof(T_offset_ptr));

            if(posix_memalign((void**)&prolongation_coarse_blocks_aligned_buffer,page_size,
                              (n_meta_blocks*8*sizeof(T_offset_ptr)/page_size+1)*page_size))
                abort();
            memcpy(prolongation_coarse_blocks_aligned_buffer,&(prolongation_coarse_blocks[0](0)),n_meta_blocks*8*sizeof(T_offset_ptr));
        }
        ~SPGrid_Master_Array_Linearizer(){
            if(b) delete[] b; 
