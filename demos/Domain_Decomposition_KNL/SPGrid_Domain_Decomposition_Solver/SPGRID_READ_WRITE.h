...
public:
    ...
    static void Read_SPGrid(const std::string& base_dir,Allocator_type*& allocator,Set_type*& set,T T_STRUCT::* field){
        RW rw=RW(); STREAM_TYPE stream_type(rw);
        T_INDEX size;
        FILE_UTILITIES::Read_From_File(stream_type,base_dir+"/size",size);
        LOG::cout<<"Size: "<<size<<std::endl;
        allocator=new Allocator_type(std_array<int,d>(size));
        set=new Set_type(allocator->Get_Array(&T_STRUCT::flags));
        Read_Block_Offsets(base_dir+"/block_offset",*set);
        Read_Data_Channel(base_dir+"/data",*allocator,*set,field);
        Read_Flags_Channel(base_dir+"/flags",*allocator,*set);
    }
