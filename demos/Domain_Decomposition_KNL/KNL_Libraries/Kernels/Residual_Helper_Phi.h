...
//#####################################################################
#ifndef __Residual_Helper_Phi__
#define __Residual_Helper_Phi__
#include <algorithm>
...
namespace SPGrid{
template<class T,int log2_struct, int d,class T_offset_ptr> class Residual_Helper_PHI;

#define SIMD_LENGTH 16
template<class T,int log2_struct,class T_offset_ptr>
class Residual_Helper_PHI<T,log2_struct,3,T_offset_ptr>
{
    enum{d=3};
    ...
    const T* const rhs;  // first input stream
...
public:
    explicit Residual_Helper_PHI(T* const r_input,const T* const u_input,const T* const rhs_input,const unsigned* const mask_input,const T_offset_ptr* const b_input,
        ...
        :r(r_input),u(u_input),rhs(rhs_input),mask(mask_input),b(b_input),
