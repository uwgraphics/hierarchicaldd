...
template <class T, int log2_struct,class T_offset_ptr> void Residual_Helper_PHI<T,log2_struct,3,T_offset_ptr>::Run_Index_Range(const int index_start,const int index_end)
{
    ...
    for(int index=index_start;index<=index_end;index++) { 
        ...
        const_block_array rhs_center = reinterpret_cast<const_block_array>(*reinterpret_cast<const T*>((unsigned long)rhs + b[index]));
        ...
        const_block_array rhs_cente_l2 = reinterpret_cast<const_block_array>(*reinterpret_cast<const T*>((unsigned long)rhs + b[index]));

        block_array r_center = reinterpret_cast<block_array>(*reinterpret_cast<T*>((unsigned long)r + b[index]));

        __m512 tmp_v,z_plus_v,z_minus_v,y_plus_v,y_minus_v,x_plus_v,x_minus_v;
        __m512 center_v,rhs_v;
        __m512i mask_v;
        ...
        for(int j=0;j<block_ysize;j+=SIMD_width/block_zsize){
            ...
            //_mm_prefetch((const char*)&rhs_center_l2[i][j][0],_MM_HINT_T1);
            ...
            rhs_v = _mm512_load_ps(&rhs_center[i][j][0]);
            __m512 result_v = _mm512_setzero_ps();
            ...
            if(_mm512_mask2int(mask_m)){
                ...
                tmp_v = _mm512_loadu_ps(&u_z_plus[i][j-1][1]);                
                z_plus_v = _mm512_loadu_ps(&u_center[i][j][1]);
                z_plus_v = _mm512_mask_blend_ps(z_plus_lo_m,tmp_v,z_plus_v);
                
                tmp_v = _mm512_loadu_ps(&u_center[i][j][-1]);
                z_minus_v = _mm512_loadu_ps(&u_z_minus[i][j+1][-1]);
                ...
                if(yspan_simd > 1){
                    ...
                    if(j+SIMD_width/block_zsize>=block_ysize){
                        ...
                        tmp_v = _mm512_loadu_ps(&u_y_plus[i][1-SIMD_width/block_zsize][0]);
                    }else{
                        tmp_v = _mm512_loadu_ps(&u_center[i][j+1][0]);
                    }
                    y_plus_v = _mm512_loadu_ps(&u_center[i][j+1][0]);
                ...
                if(yspan_simd > 1){
                    ...
                    if(j==0){
                        ...
                        y_minus_v = _mm512_loadu_ps(&u_y_minus[i][block_ysize-1][0]);
                    }else{
                         y_minus_v = _mm512_loadu_ps(&u_center[i][j-1][0]);
                    }
                ...
                result_v = _mm512_add_ps(result_v,x_minus_v);
                result_v = _mm512_mask_sub_ps(_mm512_setzero_ps(),mask_m,rhs_v,result_v);
    
                _mm512_stream_ps(&r_center[i][j][0],result_v);
            }
        }
    }
}

template class Residual_Helper_PHI<float,4,3,unsigned>;
template class Residual_Helper_PHI<float,4,3,unsigned long>;
