...
template <class T, int log2_struct,class T_offset_ptr> void Laplace_Helper_PHI<T,log2_struct,3,T_offset_ptr>::Run_Index_Range(const int index_start,const int index_end)
{
    ...
    for(int index=index_start;index<=index_end;index++) {
        ...
        for(int j=0;j<block_ysize;j+=SIMD_width/block_zsize){
            ...
            if(_mm512_mask2int(mask_m)){
                ...
                _mm512_stream_ps(&Lu_center[i][j][0],result_v);
            }
        }
    }
}

template class Laplace_Helper_PHI<float,4,3,unsigned>;
template class Laplace_Helper_PHI<float,4,3,unsigned long>;
//template class Laplace_Helper_PHI<float,5,3,unsigned long>;
//template class Laplace_Helper_PHI<float,6,3,unsigned long>;
