...
template <class T, int log2_struct,class T_offset_ptr> void Correction_Helper_PHI<T,log2_struct,3,T_offset_ptr>::Run_Index_Range(const int index_start,const int index_end)
{
    ...
    for(int index=index_start;index<=index_end;index++)
        for(int i=0;i<T_MASK::elements_per_block;i+=SIMD_width){
            unsigned* mask_ptr = reinterpret_cast<unsigned*>((unsigned long)mask + ((unsigned long)index<<12) + i * sizeof(unsigned));
            T* u_ptr = reinterpret_cast<T*>((unsigned long)u + ((unsigned long)index<<12) + i * sizeof(T));
            const T* r_ptr = reinterpret_cast<const T*>((unsigned long)r + ((unsigned long)index<<12) + i * sizeof(T));
            ...
            if(_mm512_mask2int(mask_m)){
                ...
                _mm512_stream_ps(u_ptr,u_new_v);}}
}
...
template class Correction_Helper_PHI<float,4,3,unsigned long>;
