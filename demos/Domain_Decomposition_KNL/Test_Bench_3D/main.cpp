...
int main(int argc,char *argv[])
{
    ...
    parse_args.Add_String_Argument("-iomode","output","output, input, or neither");
    ...
    std::string iomode=parse_args.Get_String_Value("-iomode");
    ...
    typedef unsigned long T_offset_ptr;
    ...
    if(mode=="no_convertion"){    
        ...
        solver.Initialize_Preconditioner(v_cycle_levels,size.Cast<T_INDEX>(),subdomain_size,sigma_levels,nVcycles,interior_smoothing,boundary_smoothing,sigma_smoothing,n_sigma_mg,iomode,output_directory);
        solver.Solve();
    }else{
        if(mode=="velocity"){
