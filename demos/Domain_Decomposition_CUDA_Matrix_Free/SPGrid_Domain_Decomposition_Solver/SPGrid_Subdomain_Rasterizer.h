...
public:
    ...
    bool Consume(const RANGE<VECTOR<int,d> >& range){
        ...
        // Recurse until we are down to the size of a single block
        if(range.Edge_Lengths()!=block_size) return true;
        if(!reference_set.IsPageActive(SPG_Flags_Array_Type::MASK::Linear_Offset(std_array<int,d>(adjusted_range.min_corner)))) return false;
        ...
        for(RANGE_ITERATOR<d> iterator(expanded_range);iterator.Valid();iterator.Next()){
            ...
            if(subdomain.Lazy_Inside(cell_index)){
                flags_local(cell_index)=reference_flags(SPG_Flags_Array_Type::MASK::Linear_Offset(std_array<int,d>(cell_index)));
