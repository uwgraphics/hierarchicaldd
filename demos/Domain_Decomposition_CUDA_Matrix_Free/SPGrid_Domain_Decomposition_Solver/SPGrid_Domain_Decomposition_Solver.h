...
public:
    ...
    SPGrid_Computations::V_Cycle_Topology<T_STRUCT_4,T,d>* v_cycle_topology;
    ...
    SPGrid_Domain_Decomposition_Solver(int number_of_threads_input=0):number_of_threads(number_of_threads_input),v_cycle_topology(0),dd_preconditioner(0){}
    ~SPGrid_Domain_Decomposition_Solver(){
        if(v_cycle_topology) delete v_cycle_topology;
    ...
    //#####################################################################
    template<typename T_STRUCT>
    static void Clear_Interface_Flags(SPGrid_Allocator<T_STRUCT,d>& flag_allocator,SPGrid_Set<typename SPGrid_Allocator<T_STRUCT,d>::template Array<unsigned>::type>& set,unsigned T_STRUCT::* flags_field){
        typedef typename SPGrid_Allocator<T_STRUCT,d>::template Array<const unsigned>::type Const_Flag_Array_Type;
        typedef typename SPGrid_Allocator<T_STRUCT,d>::template Array<T>::type Data_Array_Type;
        typedef typename SPGrid_Allocator<T_STRUCT,d>::template Array<unsigned>::mask T_MASK;
        auto flags_array = flag_allocator.Get_Array(flags_field);        
        for(SPGrid_Block_Iterator<T_MASK> iterator(set.Get_Blocks());iterator.Valid();iterator.Next_Block()){
            unsigned long offset=iterator.Offset();
            T_INDEX base_index=iterator.Index().template Cast<T_INDEX>();
            for(RANGE_ITERATOR<d> iterator(RANGE<T_INDEX>(base_index,base_index+flag_allocator.Block_Size().template Cast<T_INDEX>()-1));
    ...
    //#####################################################################
    void Initialize_Preconditioner(int v_cycle_levels,T_INDEX size,T_INDEX subdomain_size,int sigma_levels,int nVcycles,int sigma_smoothing,int n_sigma_mg){
        Clear_Interface_Flags(*(flags_field.allocator),spgrid_gemini.Set(),flags_field.field);
        ...
        splitter.Split(interface_matrix_generator,linearizer_hierarchy,v_cycle_levels,sigma_levels,number_of_threads);
        ...
        dd_preconditioner = new SPGrid_Domain_Decomposition_Preconditioner<T,T_STRUCT_4,d,T_offset_ptr>(*(flags_field.allocator),spgrid_gemini.Set(),
                                                                                                        ...
                                                                                                        nVcycles,sigma_smoothing,n_sigma_mg,number_of_threads);
