...
public:
    ...
    {
        std::cout<<"allocator.Padded_Size(): "<<allocator.Padded_Size()<<std::endl;
    }
    void Split(INTERFACE_MATRIX_GENERATOR<T,d>& interface_matrix_generator,
               std::vector<std::vector<T_Linearizer>*>& linearizer_hierarchy,const int v_cycle_levels,const int sigma_mg_level,const int number_of_threads=0){
        ...
        for(int i=0;i<subdomain_id_map.size();++i){
            ...
            while(true){
                ...
                while(!coloring_queue.empty()){
                    ...
                    for(int axis=1;axis<=d;++axis){
                        for(int v=-1;v<=1;v+=2){
                            ...
                            if(flags_ptr(neighbor_offset)&SPGrid_Solver_Cell_Type_Active){
                ...
                SPGrid_Computations::V_Cycle_Topology<T_STRUCT,T,d> v_cycle_topology(sub_allocator,sub_set,domain_size,v_cycle_levels,&T_STRUCT::flags);
                ...
                for(int level=0;level<v_cycle_levels;++level){
                    ...
                    if(level==0) std::cout<<"Top blocks: "<<subdomain_blocks[level].size()<<std::endl;                  
                    ...
                    for(unsigned int i=0;i<subdomain_blocks[level].size();++i){
                        memcpy(subdomain_linearizer[level].data+i*T_Linearizer::page_size,(char*)&flags_ptr(subdomain_blocks[level][i]),T_Linearizer::page_size);}
                    for(unsigned int i=0;i<interface_blocks.size();++i){
                        memcpy(subdomain_linearizer[level].interface_data+i*T_Linearizer::page_size,(char*)&flags_ptr(interface_blocks[i]),T_Linearizer::page_size);}
                }
            ...
            std::cout<<"Subdomain splitted: "<<subdomain_splited<<std::endl;            
#else
            SPG_Flags_Array_Type flags_ptr=subdomain_allocator.Get_Array(&T_STRUCT::flags);
            for(SPGrid_Block_Iterator<T_MASK> iterator(subdomain_set.Get_Blocks());iterator.Valid();iterator.Next()){
                unsigned long offset=iterator.Offset();
