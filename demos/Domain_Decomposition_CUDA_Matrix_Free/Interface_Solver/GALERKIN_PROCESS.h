...
// #############################################################################
public:
    static Eigen::Matrix<T,1<<d,1<<d> Galerkin_Coarsen(const Set_type_solver& set,const std::vector<Eigen::Matrix<T,1<<d,1<<d> >& prolongation_matrices,const ARRAY<int,T_INDEX>& index_map,const T_INDEX& cell_index,int level){
