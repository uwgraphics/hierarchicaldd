...
int main(int argc,char *argv[])
{
    ...
    parse_args.Add_String_Argument("-mode","velocity","velocity or rhs");
    parse_args.Add_String_Argument("-swap","false","does we swap memory out of the GPUs");
    ...
    if(mode=="no_convertion"){
        ...
        solver.Initialize_Preconditioner(v_cycle_levels,size.Cast<T_INDEX>(),subdomain_size,sigma_levels,nVcycles,sigma_smoothing,n_sigma_mg);
        solver.Solve();
    }else{
        if(mode=="velocity"){
    ...
    solver.Copy(allocator_simulation,set_simulation,&T_STRUCT_4::ch1,b_field);
    delete SPGrid_hierarchy;
    solver.Initialize_Preconditioner(v_cycle_levels,size.Cast<T_INDEX>(),subdomain_size,sigma_levels,nVcycles,sigma_smoothing,n_sigma_mg);
    
    SPGrid_Domain_Decomposition_Solver<T,T_STRUCT_4,T_STRUCT_SIMULATION,d,T_offset_ptr>::Clear_Non_Active_Values<T_STRUCT_4>(solver.spgrid_gemini.First_Allocator(),solver.spgrid_gemini.Second_Allocator(),solver.spgrid_gemini.Set(),&T_STRUCT_4::ch1,&T_STRUCT_4::flags);
    solver.Solve();
    LOG::Finish_Logging();
    return 0;
}
