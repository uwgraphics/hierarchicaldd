...
class RUNGEKUTTA_CORE<VECTOR<T,d1> >:public NONCOPYABLE
{
public:
    int order; // 1, 2 or 3
    T dt; // size of time step
    int substep; // current substep
    ARRAY_VIEW<VECTOR<T,d1> > u; // variable being advanced in time
    ARRAY<VECTOR<T,d1> > u_copy; // a copy of the variable
protected:
    T time; // current time
    int pseudo_time; // real time or pseudo time - for boundary conditions
public:
    ...
    template<class TV>
    RUNGEKUTTA_CORE(TV& u_input)
        :u(Scalar_View(u_input)),u_copy(u)
    {
        Set_Order();Set_Time();Real_Time();
    }
...
template<class T,int d,int d1>
class RUNGEKUTTA<ARRAY<VECTOR<T,d1> ,FACE_INDEX<d> > >:public RUNGEKUTTA_CORE<VECTOR<T,d1> >
{
private:
    ARRAY<VECTOR<T,d1>,FACE_INDEX<d> >& u;
public:

    RUNGEKUTTA(ARRAY<VECTOR<T,d1>,FACE_INDEX<d> >& u)
        :RUNGEKUTTA_CORE<VECTOR<T,d1> >(u),u(u)
    {}

    static RUNGEKUTTA* Create(ARRAY<VECTOR<T,d1>,FACE_INDEX<d> >& u,const int order,const T dt,const T time)
    {RUNGEKUTTA* runge_kutta=new RUNGEKUTTA(u);runge_kutta->Set_Order(order);runge_kutta->Set_Time(time);
    runge_kutta->dt=dt;runge_kutta->substep=0;return runge_kutta;}
