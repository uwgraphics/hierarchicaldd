...
//#####################################################################
void PARSE_ARGS::
Set_Extra_Arguments(int num, const std::string& synopsis,const std::string& desc) // num=-1 for arbitrary extra arguments
{
    num_expected_extra_args=(num!=-1)?num_expected_extra_args+1:num;if(synopsis.length())extra_args_synopsis_list.Append(synopsis);if(desc.length())extra_args_desc_list.Append(desc);
}
//#####################################################################
...
Print_Usage(bool do_exit) const
{
    ...
    for(i=1;i<=extra_args_synopsis_list.m;i++) LOG::cerr<<extra_args_synopsis_list(i)<<" ";LOG::cerr<<std::endl;
    int width=0,len;
    for(i=1;i<=arg_data_list.m;i++){len=(int)arg_data_list(i).str.length();if(len>width)width=len;}
    for(i=1;i<=extra_args_desc_list.m;i++){len=(int)extra_args_synopsis_list(i).length();if(len>width)width=len;}
    for(i=1;i<=arg_data_list.m;i++){arg_data_list(i).Print_Description(width+2);LOG::cerr<<std::endl;}
    for(i=1;i<=extra_args_desc_list.m;i++){LOG::cerr<<extra_args_synopsis_list(i);for(unsigned j=1;j<=width+2-extra_args_synopsis_list(i).length();j++) LOG::cerr<<" ";LOG::cerr<<extra_args_desc_list(i)<<std::endl;} 
