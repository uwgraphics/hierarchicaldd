...
//#####################################################################
    virtual void Fill_Ghost_Cells(const T_GRID& grid,const T_ARRAYS_DIMENSION_T2& u,T_ARRAYS_DIMENSION_T2& u_ghost,const T dt,const T time,const int number_of_ghost_cells) PHYSBAM_OVERRIDE;
...
Fill_Ghost_Cells(const T_GRID& grid,const T_ARRAYS_DIMENSION_T2& u,T_ARRAYS_DIMENSION_T2& u_ghost,const T dt,const T time,const int number_of_ghost_cells)
{
    ...
    for(int side=1;side<=2*T_GRID::dimension;++side) boundaries[side]->Fill_Single_Ghost_Region(grid,u_ghost,side,dt,time,regions(side)); 
}
//#####################################################################
