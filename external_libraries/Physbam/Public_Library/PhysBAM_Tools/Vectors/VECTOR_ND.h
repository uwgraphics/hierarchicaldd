//#####################################################################
// Copyright 2002-2014, Ronald Fedkiw, Geoffrey Irving, Frank Losasso, Igor Neverov, Craig Schroeder, Rahul Sheth, Tamar Shinar, Eftychios Sifakis, Joseph Teran, Rachel Weinstein, Zahid Hossain
...
public:
    ...
    for(int i=1;i<n;++i) if(max<x[i]){max=x[i];arg_max=i+1;};
    return arg_max;}
    ...
    bool operator==(const VECTOR_ND<T> &v) const
    {assert(v.Size()==Size());
    for(int i=1;i<=Size();i++) if((*this)(i)!=v(i)) return false;
    return true;}

    bool All_Less_Equal(const VECTOR_ND<T> &v) const
    {for(int i=1;i<Size();i++){if((*this)(i)>v(i)) return false;}return true;}

    void Clamp_In_Place(const T vmin,const T vmax)
    {for(int i=0;i<Size();i++){x[i]=clamp(x[i],vmin,vmax);}}
