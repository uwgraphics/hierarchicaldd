...
public:
    ...
    void Fill_Ghost_Cells(const T_GRID& grid,const T_ARRAYS_T2& u,T_ARRAYS_T2& u_ghost,const T dt,const T time,const int number_of_ghost_cells_input) PHYSBAM_OVERRIDE
    {
        ...
        for(int side=1;side<=T_GRID::number_of_faces_per_cell;side++){int axis=(side+1)/2;
            DOMAIN_ITERATOR_THREADED_ALPHA<T_BOUNDARY_T2,TV>(regions(side),&thread_queue,axis%TV::dimension+1).template Run<const T_GRID&,T_ARRAYS_T2&,const int,const T,const T>(boundary,&T_BOUNDARY_T2::Fill_Single_Ghost_Region_Threaded,grid,u_ghost,side,dt,time);}
    }
