...
MPI_GRID(T_GRID& local_grid_input,const int number_of_ghost_cells_input,const bool skip_initialization,const TV_INT& processes_per_dimension,const TV_BOOL& periodic_input,
    ...
    :local_grid(local_grid_input),number_of_ghost_cells(number_of_ghost_cells_input),comm(0),group(group_input),current_tag(0),periodic(periodic_input),ignore_boundary_faces(false),include_ghost_cells_in_effective_local_domain(false)
{
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    std::stringstream ss;
    ss<<"dividing domain into "<<count.x<<" by "<<count.y<<" processor grid"<<std::endl;
    LOG::filecout(ss.str());
#endif
    boundaries.Resize(2);
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    std::stringstream ss;
    ss<<"dividing domain into "<<count.x<<" by "<<count.y<<" by "<<count.z<<" processor grid"<<std::endl;
    LOG::filecout(ss.str());
#endif
    boundaries.Resize(3);
    Split_Dimension(m,count.x,boundaries(1));
...
Sync_Common_Cell_Weights_From(ARRAY<ARRAY<PAIR<TV_INT,T> >,TV_INT>& weights_to,ARRAY<ARRAY<PAIR<TV_INT,int> >,TV_INT>& weights_from,const int ghost_cells)
{
    ...
    for(int i=1;i<=number_of_processes;i++){int position=0;
        for(int j=1;j<=receive_per_proc(i).m;j++){
            ...
            if(weights_to.Valid_Index(data.x-local_to_global_offset) && weights_from.Valid_Index(target_cell)){
...
Sync_Common_Cell_Weights_To(ARRAY<ARRAY<PAIR<TV_INT,T> >,TV_INT>& weights_to,ARRAY<ARRAY<PAIR<TV_INT,int> >,TV_INT>& weights_from,const int ghost_cells)
{
    ...
    for(int i=1;i<=number_of_processes;i++){int position=0;
        for(int j=1;j<=receive_per_proc(i).m;j++){
            ...
            if(weights_to.Valid_Index(data.y-local_to_global_offset) && weights_from.Valid_Index(target_cell)){
...
#define INSTANTIATION_HELPER(T,T_GRID,T_REAL_GRID) \
    ...
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Face_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::FACE_ARRAYS::REBIND<int>::TYPE&,const int) const; \
    ...
    template void MPI_GRID<T_GRID >::Exchange_Boundary_Node_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::ARRAYS_BASE::REBIND<int>::TYPE&,const int) const; \
    ...
    template void MPI_GRID<T_GRID >::Average_Common_Face_Data(const MPI_GRID_POLICY<T_REAL_GRID>::MPI_GRID&,GRID_ARRAYS_POLICY<T_REAL_GRID>::FACE_ARRAYS::REBIND<int>::TYPE&) const; \
...
#define INSTANTIATION_HELPER_UNIFORM(T,T_GRID,d)     \
    ...
    template void MPI_GRID<T_GRID >::Reduce_Add(const ARRAY<int>&,ARRAY<int>&) const; \
