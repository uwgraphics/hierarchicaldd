...
{
    if(thread_grid){Parallel_Solve_Threaded(A,x,b,tolerance,recompute_preconditioner);return;}
...
//#####################################################################
// Function Parallel_Solve_Threaded
//#####################################################################
template<class T_GRID> void PCG_SPARSE_MPI<T_GRID>::
Parallel_Solve_Threaded(SPARSE_MATRIX_FLAT_NXN<T>& A,VECTOR_ND<T>& x,VECTOR_ND<T>& b,const T tolerance,const bool recompute_preconditioner)
{
    int local_n=A.n,interior_n=partition.interior_indices.Size()+1;
    int global_n=Global_Sum_Threaded(interior_n);
    T global_tolerance=Global_Max_Threaded(tolerance);
    int desired_iterations=global_n;if(pcg.enforce_compatibility) desired_iterations--;if(pcg.maximum_iterations) desired_iterations=min(desired_iterations,pcg.maximum_iterations);
    ...
    // adjust x for the null space
    if(pcg.enforce_compatibility && pcg.remove_null_space_solution_component) x_interior-=(T)(Global_Sum_Threaded(x_interior.Sum_Double_Precision())/global_n);

    // find initial residual, r=b-Ax - reusing b for the residual
    Fill_Ghost_Cells_Threaded(x);
    A.Times(x,temp);b_interior-=temp_interior;
    if(pcg.enforce_compatibility) b_interior-=(T)(Global_Sum_Threaded(b_interior.Sum_Double_Precision())/global_n);



    if(Global_Max_Threaded(b_interior.Max_Abs())<=global_tolerance){
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(pcg.show_results) LOG::filecout("NO ITERATIONS NEEDED\n");
#endif
        return;}
    ...
    for(int iteration=1;;iteration++){
        ...
        // for Neumann boundary conditions only, make sure z sums to zero
        if(pcg.enforce_compatibility) z_interior-=(T)(Global_Sum_Threaded(z_interior.Sum_Double_Precision())/global_n);

        // update search direction
        rho_old=rho;rho=Global_Sum_Threaded(VECTOR_ND<T>::Dot_Product_Double_Precision(z_interior,b_interior));
        T beta=0;if(iteration==1) p_interior=z_interior;else{beta=(T)(rho/rho_old);for(int i=1;i<=interior_n;i++) p_interior(i)=z_interior(i)+beta*p_interior(i);} // when iteration=1, beta=0

        // update solution and residual
        Fill_Ghost_Cells_Threaded(p);
        A.Times(p,temp);
        T alpha=(T)(rho/Global_Sum_Threaded(VECTOR_ND<T>::Dot_Product_Double_Precision(p_interior,temp_interior)));
        for(int i=1;i<=interior_n;i++){x_interior(i)+=alpha*p_interior(i);b_interior(i)-=alpha*temp_interior(i);}
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        T residual=Global_Max_Threaded(b_interior.Max_Abs());
...
//#####################################################################
// Function Fill_Ghost_Cells_Threaded
//#####################################################################
template<class T_GRID> void PCG_SPARSE_MPI<T_GRID>::
Fill_Ghost_Cells_Threaded(VECTOR_ND<T>& x)
{
    ...
    // Send out the column values that we owe other people
    for(int s=1;s<=partition.number_of_sides;s++){
         int neighbor_rank=partition.neighbor_ranks(s);
         if(partition.neighbor_ranks(s)!=-1){
             THREAD_PACKAGE pack(partition.boundary_indices(s).m*sizeof(T));
             pack.send_tid=thread_grid->tid;pack.recv_tid=neighbor_rank+1;int position=0;
             for(int i=1;i<=partition.boundary_indices(s).m;i++){*(T*)(&pack.buffer(position+1))=x(partition.boundary_indices(s)(i));position+=sizeof(T);}
    ...
    for(int s=1;s<=partition.number_of_sides;s++){
        ...
        if(partition.neighbor_ranks(s)!=-1){
            int start=partition.ghost_indices(s).min_corner;
            for(int buf=1;buf<=buffers->m;buf++){
                if(thread_grid->tid!=(*buffers)(buf).recv_tid || neighbor_rank!=(*buffers)(buf).send_tid-1) continue;
                int position=0;
                for(int i=1;i<=partition.ghost_indices(s).Size()+1;i++){
                    x(start+i-1)=*(T*)(&(*buffers)(buf).buffer(position+1));
                    position+=sizeof(T);}}}}
    pthread_barrier_wait(barr);
    pthread_mutex_lock(lock);
    buffers->m=0;
