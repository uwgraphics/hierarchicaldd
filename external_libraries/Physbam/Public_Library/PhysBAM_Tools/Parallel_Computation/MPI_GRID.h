...
public:
    ...
    bool include_ghost_cells_in_effective_local_domain;
    ...
    RANGE<TV> Effective_Local_Domain() const
    {if(!include_ghost_cells_in_effective_local_domain) return local_grid.domain;
    RANGE<TV> domain=local_grid.domain;
    for(int n=1;n<=T_GRID::number_of_neighbors_per_node;n++) if(side_neighbor_ranks(n)==-2){
        TV_INT direction=T_GRID::Node_Neighbor(TV_INT(),n);
        int axis=direction.Dominant_Axis();
        if(direction(axis)==1) domain.max_corner(axis)+=number_of_ghost_cells*local_grid.DX()(axis);
        if(direction(axis)==-1) domain.min_corner(axis)-=number_of_ghost_cells*local_grid.DX()(axis);}
    return domain;}
