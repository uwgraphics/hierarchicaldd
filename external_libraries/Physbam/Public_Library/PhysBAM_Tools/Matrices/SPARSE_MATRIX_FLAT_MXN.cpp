...
//#####################################################################
template<class T> SPARSE_MATRIX_FLAT_MXN<T> SPARSE_MATRIX_FLAT_MXN<T>::
Concatenate_Columns(const SPARSE_MATRIX_FLAT_MXN<T>& B) const
{
    assert(m==B.m);
    SPARSE_MATRIX_FLAT_MXN<T> result;
    ARRAY<int> row_lengths(m);
    for(int row=1;row<=m;row++)
        row_lengths(row)=offsets(row+1)-offsets(row)+B.offsets(row+1)-B.offsets(row);
    result.Set_Row_Lengths(row_lengths);
    result.n=n+B.n;
    for(int row=1;row<=m;row++){
        for(int index=offsets(row);index<offsets(row+1);index++)
            result.Set_Element(row,A(index).j,A(index).a);
        for(int index=B.offsets(row);index<B.offsets(row+1);index++)
            result.Set_Element(row,B.A(index).j+n,B.A(index).a);}
    return result;
}
//#####################################################################
// Function Concatenate_Rows
