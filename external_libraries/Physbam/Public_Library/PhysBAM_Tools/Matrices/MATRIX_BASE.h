//#####################################################################
// Copyright 2007-2013, Ronald Fedkiw, Geoffrey Irving, Igor Neverov, Craig Schroeder, Rahul Sheth, Tamar Shinar, Eftychios Sifakis, Huamin Wang, Rachel Weinstein.
...
template<class T,class T_MATRIX> struct MATRIX_INFO<MATRIX_VIEW<T,T_MATRIX> >{typedef VECTOR_ND<T> LEFT_VECTOR;typedef VECTOR_ND<T> RIGHT_VECTOR;typedef MATRIX_VIEW<T,T_MATRIX> TRANSPOSE;};
template<class T,class T_MATRIX,class T_VECTOR> struct MATRIX_INFO<GENERAL_MATRIX<T,T_MATRIX,T_VECTOR> >{typedef typename T_MATRIX::LEFT_VECTOR LEFT_VECTOR;typedef typename T_MATRIX::RIGHT_VECTOR RIGHT_VECTOR;typedef T_MATRIX TRANSPOSE;};
...
template<class T_MATRIX,class T_VECTOR> struct VECTOR_TYPE<LAZY_MATRIX_VECTOR_PRODUCT<T_MATRIX,T_VECTOR> > {typedef typename VECTOR_TYPE<T_VECTOR>::TYPE TYPE;};
...
public:
    ...
    template<class T_VECTOR>
    LAZY_MATRIX_VECTOR_PRODUCT<T_MATRIX,T_VECTOR> Lazy_Multiply(const T_VECTOR& vector_in) const
    {vector_in.Static_Assert_Not_Small();return LAZY_MATRIX_VECTOR_PRODUCT<T_MATRIX,T_VECTOR>(this->Derived(),vector_in.Derived());}
...
//#####################################################################
    ...
    template<class T_MATRIX1,class T_MATRIX2> void In_Place_Householder_QR_Factorization_With_Column_Pivoting(MATRIX_BASE<T,T_MATRIX1>& Q,MATRIX_BASE<T,T_MATRIX2>& P,const T tolerance=(T)1e-4); //this=A,R
    template<class T_MATRIX1,class T_MATRIX2> void In_Place_Householder_QR_Factorization_With_Full_Pivoting(MATRIX_BASE<T,T_MATRIX1>& Q,MATRIX_BASE<T,T_MATRIX2>& P,const T tolerance=(T)1e-4); //this=A,R
    template<class T_VECTOR1,class T_VECTOR2> void Robust_Householder_QR_Solve(VECTOR_BASE<T,T_VECTOR1>& x,const VECTOR_BASE<T,T_VECTOR2>& b,const bool use_full_pivoting=false,const T tolerance=(T)1e-4); //this=A
...
{output_stream<<std::endl;for(int i=1;i<=A.Rows();i++){output_stream<<"|";for(int j=1;j<=A.Columns();j++){output_stream<<std::setw(16)<<A(i,j);}output_stream<<"|"<<std::endl;}}
...
#include <PhysBAM_Tools/Matrices/MATRIX_EXPRESSION.h>
#endif
