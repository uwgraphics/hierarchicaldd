...
public:
    ...
    while(photon_map.Light_Emission_Quota_Remains()){
        ...
        world.random.Set_Seed(abs((int)(random->Get_Uniform_Number((T)0,(T)std::numeric_limits<unsigned int>::max()))),tid);

        T xi_x=world.random.Get_Uniform_Number((T)0,(T)1,tid),xi_y=world.random.Get_Uniform_Number((T)0,(T)1,tid),xi_z=world.random.Get_Uniform_Number((T)0,(T)1,tid);
        T probability;
        VECTOR<T,3> sample_location=Sample_Point_In_Volume(xi_x,xi_y,xi_z,probability);VECTOR<T,3> sample_direction(world.random.template Get_Direction<VECTOR<T,3> >(tid));
