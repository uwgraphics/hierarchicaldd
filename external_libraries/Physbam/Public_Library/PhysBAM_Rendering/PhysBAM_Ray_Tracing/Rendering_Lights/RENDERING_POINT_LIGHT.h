...
public:
    ...
    while(photon_map.Light_Emission_Quota_Remains()){
        ...
        world.random.Set_Seed((unsigned int)abs(random->Get_Uniform_Number((T)0,(T)std::numeric_limits<unsigned int>::max())),tid);
        photons_emitted++;
        VECTOR<T,3> direction(world.random.template Get_Direction<VECTOR<T,3> >(tid));
