...
public:
    ...
    if(use_stratified_sampling_on_photon_emit){
        ...
        while(true) for(int u=0;u<u_samples;u++)for(int v=0;v<v_samples;v++)for(int theta=0;theta<theta_samples;theta++)for(int phi=0;phi<phi_samples;phi++){
            ...
            world.random.Set_Seed(abs((int)random->Get_Uniform_Number((T)0,(T)1)),tid);
            T u_jitter_offset=world.random.Get_Uniform_Number((T)-.5,(T).5,tid),v_jitter_offset=world.random.Get_Uniform_Number(T(-.5),T(.5),tid);
            T xi_1_jitter_offset=world.random.Get_Uniform_Number((T)-.5,(T).5,tid),xi_2_jitter_offset=world.random.Get_Uniform_Number(T(-.5),T(.5),tid);
        ...
        world.random.Set_Seed(abs((int)random->Get_Uniform_Number((T)0,(T)1)),tid);
        VECTOR<T,3> point_on_light=normal*small_number*4+position+world.random.Get_Uniform_Number(T(0),T(1),tid)*u_direction+world.random.Get_Uniform_Number(T(0),T(1),tid)*v_direction;
        VECTOR<T,3> direction=world.Random_Reflected_Direction_In_Hemisphere(normal,world.random.Get_Uniform_Number(T(0),T(1),tid),world.random.Get_Uniform_Number(T(0),T(1),tid));
