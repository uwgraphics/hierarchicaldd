    ...
    global_photon_map(0),caustic_photon_map(0),volume_photon_map(0),threads(1)
{
    ...
    Clean_Scene();
    //randoms.Resize(1);randoms(1)=new RANDOM_NUMBERS<T>(23123);
...
public:
    ...
    for(int i=1;i<=samples.m;i++){
        ...
        if(i==samples.m){progress.Progress();pthread_mutex_lock(mutex);pthread_mutex_unlock(mutex);}}
    }
...
template<class T>
class RAY_TRACING_TASK:public THREAD_QUEUE::TASK
{
    RENDER_WORLD<T>& world;
    PROGRESS_INDICATOR& progress;
    pthread_mutex_t* mutex;
    VECTOR<int,2> min_corner;
    VECTOR<int,2> box_size;
    VECTOR<int,2> lengths;
    int thread_number;
    int tid;

public:
    RAY_TRACING_TASK(RENDER_WORLD<T>& world,PROGRESS_INDICATOR& progress,pthread_mutex_t* mutex,const VECTOR<int,2>& min_corner,const VECTOR<int,2>& box_size,const VECTOR<int,2>& lengths,int thread_number,int tid)
        :world(world),progress(progress),mutex(mutex),min_corner(min_corner),box_size(box_size),lengths(lengths),thread_number(thread_number),tid(tid)
    {}

    virtual void Run(const int thread_id)
    {
        RENDERING_RAY<T> dummy_root;
        ARRAY<typename FILM<T>::SAMPLE> samples;
        int counter=0;int remainder=tid-1;  ////tid start from 1
        int c=0;
        for(int box_x=0;box_x*box_size.x<lengths.x;box_x++) for(int box_y=0;box_y*box_size.y<lengths.y;box_y++){
            if(tid==1)progress.Progress();
            counter++;
            if(counter%thread_number==remainder){
                samples.Remove_All();c++;
                VECTOR<int,2> lower_corner=VECTOR<int,2>(box_x*box_size.x,box_y*box_size.y)+min_corner;
                world.camera.film.Generate_Samples(RANGE<VECTOR<int,2> >(lower_corner,lower_corner+box_size),world.camera,samples);
                for(int i=1;i<=samples.m;i++){
                    typename FILM<T>::SAMPLE& sample=samples(i);
                    RENDERING_RAY<T> ray(RAY<VECTOR<T,3> >(world.camera.position,sample.world_position-world.camera.position),1,world.Point_Inside_Object(world.camera.position));
                    dummy_root.recursion_depth=0;dummy_root.current_object=world.ether;
                    sample.radiance=world.Cast_Ray(ray,dummy_root,tid);
    ...
    void Commit(){progress.Progress();}
};
#endif

template<class T> void RENDER_WORLD<T>::
Render(const RANGE<VECTOR<int,2> >& pixels, PROGRESS_INDICATOR &progress, bool show_progress/*=true*/)
{
    RENDERING_RAY<T> dummy_root;
    VECTOR<int,2> box_size=camera.film.Samples_Extent();
    VECTOR<int,2> lengths=pixels.Edge_Lengths();
    ARRAY<typename FILM<T>::SAMPLE> samples;
    progress.Initialize((lengths.x/box_size.x)*(lengths.y/box_size.y));
   
    if(threads>1){
#ifdef USE_PTHREADS
        LOG::cout<<"multi-thread ray tracing: "<<threads<<" threads."<<std::endl;
        pthread_mutex_t mutex;
        pthread_mutex_init(&mutex,0);
        THREAD_QUEUE task_queue(threads);
        for(int tid=1;tid<=threads;tid++){
            RAY_TRACING_TASK<T>* task=new RAY_TRACING_TASK<T>(*this,progress,&mutex,pixels.min_corner,box_size,lengths,threads,tid);
    ...
    else{LOG::cout<<"single-thread ray tracing."<<std::endl;
        for(int box_x=0;box_x*box_size.x<lengths.x;box_x++) for(int box_y=0;box_y*box_size.y<lengths.y;box_y++){
            samples.Remove_All();
            ...
            if(show_progress)progress.Progress();
            for(int i=1;i<=samples.m;i++){
                typename FILM<T>::SAMPLE& sample=samples(i);
...
//#####################################################################
template<class T> VECTOR<T,3> RENDER_WORLD<T>::
Cast_Ray(RENDERING_RAY<T>& ray,const RENDERING_RAY<T>& parent_ray,const int tid/*=1*/)
{
    ...
    if(!object) return Attenuate_Ray(ray,background_shader->Shade_Surface_Using_Direct_Illumination(ray,*ray.current_object,*ray.current_object,*ray.current_object,TV(),TV(1,0,0)),tid);
    ...
    TV color=object->material_shader->Shade_Surface_Using_Direct_Illumination(ray,*ray.current_object,*entering_object,*object,intersection_point,same_side_normal,tid);
    ...
    return Attenuate_Ray(ray,color,tid);
}
//#####################################################################
...
//#####################################################################
template<class T> VECTOR<T,3> RENDER_WORLD<T>::
Incident_Light(RENDERING_RAY<T> ray,const RENDERING_LIGHT<T>& light,const RENDERING_RAY<T>& full_ray,const RENDERING_RAY<T>& parent_ray,const int tid/*=1*/)
{
    if(parent_ray.recursion_depth>=ray_depth_limit-1 || ray.ray_contribution<=ray_contribution_limit) return TV();
    ...
    return Attenuate_Light_Ray(ray,light,incident_light_color,tid);
}
//#####################################################################
// Incident_Light 
//#####################################################################
...
//#####################################################################
template<class T> VECTOR<T,3> RENDER_WORLD<T>::
Attenuate_Ray(RENDERING_RAY<T>& ray,const TV& color,const int tid/*=1*/)
{
    TV attenuated_color=color;
    for(int i=1;i<=volumetric_objects.m;i++)
        if(volumetric_objects(i)->priority>=ray.current_object->priority)
            attenuated_color=volumetric_objects(i)->volumetric_shader->Attenuate_Color(ray,*volumetric_objects(i),attenuated_color,tid);
...
//#####################################################################
template<class T> VECTOR<T,3> RENDER_WORLD<T>::
Attenuate_Light_Ray(RENDERING_RAY<T>& ray,const RENDERING_LIGHT<T>& light,const TV& color,const int tid/*=1*/)
{
    TV attenuated_light_color=color;
    for(int i=1;i<=volumetric_objects.m;i++) 
        if(volumetric_objects(i)->priority>=ray.current_object->priority)
            attenuated_light_color=volumetric_objects(i)->volumetric_shader->Attenuate_Light(ray,*volumetric_objects(i),light,attenuated_light_color,tid);
...
Prepare_For_Forward_Ray_Trace()
{
    const int tid=1;////TODO: threading the precomputation, currently just do everything in tid=1
    for(int i=1;i<=standard_objects.m;i++)standard_objects(i)->Preprocess_Efficiency_Structures(*this,tid);
    for(int i=1;i<=volumetric_objects.m;i++)volumetric_objects(i)->Preprocess_Efficiency_Structures(*this,tid);
