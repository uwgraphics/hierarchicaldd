...
#include <PhysBAM_Tools/Random_Numbers/THREADED_RANDOM_NUMBERS.h>
...
public:
    ...
    CAMERA<T> camera;
    //ARRAY<RANDOM_NUMBERS<T>* > randoms;  ////for threading
    THREADED_RANDOM_NUMBERS<T> random;
    ...
    void Initialize_Threads(int threads_input)
    {threads=threads_input;random.Initialize_Threads(threads,881234);}
...
//#####################################################################
    void Render(const RANGE<VECTOR<int,2> >& pixels, PROGRESS_INDICATOR &progress, bool show_progress=true);
    ...
    TV Cast_Ray(RENDERING_RAY<T>& ray,const RENDERING_RAY<T>& parent_ray,const int tid=1);
    TV Cast_Ray_For_Photon_Gather(RENDERING_RAY<T>& ray,const RENDERING_RAY<T>& parent_ray);
    TV Incident_Light(RENDERING_RAY<T> ray, const RENDERING_LIGHT<T>& light, const RENDERING_RAY<T>& full_ray,const RENDERING_RAY<T>& parent_ray,const int tid=1);
...
#endif
 private:
    TV Attenuate_Ray(RENDERING_RAY<T>& ray,const TV& color,const int tid=1);
    TV Attenuate_Light_Ray(RENDERING_RAY<T>& ray,const RENDERING_LIGHT<T>& light,const TV& color,const int tid=1);
