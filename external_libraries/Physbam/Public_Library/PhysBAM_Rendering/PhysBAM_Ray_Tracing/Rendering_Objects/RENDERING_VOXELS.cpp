...
Precompute_Light_Data(bool use_fast_precomputation,RENDER_WORLD<T>& world,const int tid/*=1*/)
{
    ...
    for(int light_index=1;light_index<=lights.m;light_index++){
        ...
        if(sample_array.m==1 && sample_array(1).semi_infinite){
            if(use_fast_precomputation){
                for(int i=1;i<=location_list.m;i++)distance_squared_to_light(i)=-VECTOR<T,3>::Dot_Product(location_list(i),sample_array(1).direction);
                Sort(node_indirection,Indirect_Comparison(distance_squared_to_light));}
            for(int i=1;i<=location_list.m;i++){
                RAY<VECTOR<T,3> > rr(location_list(node_indirection(i)),sample_array(1).direction);
                RENDERING_RAY<T> ray_to_light(rr,1,this);
                VECTOR<T,3> light_color=world.Incident_Light(ray_to_light,*lights(light_index),ray_to_light,parent_ray,tid);
            ...
            for(int i=1;i<=location_list.m;i++){
                ...
                RENDERING_RAY<T> ray_to_light(rr,1,this);
                VECTOR<T,3> light_color=world.Incident_Light(ray_to_light,*lights(light_index),ray_to_light,parent_ray,tid);
                //VECTOR<T,3> light_color=this->volumetric_shader->Attenuate_Light(ray_to_light,*this,*lights(light_index),(*lights(light_index)).Emitted_Light(ray_to_light),tid);
                Set_Precomputed_Light_Data(node_indirection(i),light_index,light_color);
                if(use_fast_precomputation)Set_Precomputed_Light_Valid(node_indirection(i),light_index,true);
                progress.Progress();}}}
