...
public:
    ...
    for(int i=1;i<=shaders.m;i++) 
        total_radiance+=shaders(i)->Shade_Surface_Using_Direct_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,tid);
    ...
    {VECTOR<T,3> total_radiance(0,0,0);
    for(int i=1;i<=shaders.m;i++)
        total_radiance+=shaders(i)->Shade_Light_Ray(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,light,full_ray,tid);
    return total_radiance;}
