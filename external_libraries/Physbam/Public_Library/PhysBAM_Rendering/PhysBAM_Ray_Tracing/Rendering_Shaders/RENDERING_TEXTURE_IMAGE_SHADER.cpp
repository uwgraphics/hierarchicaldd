...
{
    int ghost_cells=3;
    grid.Initialize(m,n,RANGE<VECTOR<T,2> >::Unit_Box()*scaling,true);pixels.Resize(grid.Domain_Indices(ghost_cells));
     ...
     const RENDERING_OBJECT<T>& intersection_object,const VECTOR<T,3>& intersection_point,const VECTOR<T,3>& same_side_normal,const int tid/*=1*/) const
{
    ...
    if(wrap_s) s-=scaling*floor(s/scaling);else clamp(s,(T)0,scaling);
    if(wrap_t) t-=scaling*floor(t/scaling);else clamp(t,(T)0,scaling);
