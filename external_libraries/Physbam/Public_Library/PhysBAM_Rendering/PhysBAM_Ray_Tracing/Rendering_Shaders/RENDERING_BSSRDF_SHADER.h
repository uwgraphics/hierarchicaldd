...
//#####################################################################
    ...
    VECTOR<T,3> Calculate_Surface_Irradiance(const VECTOR<T,3>& point,const VECTOR<T,3>& same_side_normal,RENDER_WORLD<T>& world,const RENDERING_OBJECT<T>& intersection_object,const int tid=1);
    VECTOR<T,3> Shade_Surface_Using_Direct_Illumination(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& exiting_object,const RENDERING_OBJECT<T>& entering_object,const RENDERING_OBJECT<T>& intersection_object,const VECTOR<T,3>& intersection_point,const VECTOR<T,3>& same_side_normal,,const int tid=1) const PHYSBAM_OVERRIDE;
//#####################################################################
};
}
#endif
