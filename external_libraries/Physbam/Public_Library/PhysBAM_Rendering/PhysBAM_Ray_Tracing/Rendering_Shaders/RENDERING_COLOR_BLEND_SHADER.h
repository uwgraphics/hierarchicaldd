...
public:
    ...
    for(int i=1;i<=shaders.m;i++)color_accumulator+=weights(i)*shaders(i)->Shade_Light_Ray(ray,exiting_object,entering_object,intersection_object,intersection_point,
        same_side_normal,light,full_ray,tid);
    return color_accumulator;}
    ...
    {T r=world.random.Get_Uniform_Number((T)0,total_weight,tid);
    for(int i=1;i<=shaders.m-1;i++){
        if(r<=weights(i)){shaders(i)->Receive_Photon(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,power,type,diffuse_bounces,specular_bounces,tid);return;}
        else r-=weights(i);}
    shaders(shaders.m)->Receive_Photon(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,power,type,diffuse_bounces,specular_bounces,tid);}
