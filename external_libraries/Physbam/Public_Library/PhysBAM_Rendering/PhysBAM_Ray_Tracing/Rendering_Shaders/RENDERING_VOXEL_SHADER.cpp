...
Attenuate_Color(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& object,const TV& color,const int tid/*=1*/)
{
    ...
    while(!last_segment){
        ...
        T current_volumetric_step=voxel_object->Volumetric_Integration_Step(RAY<TV>(current_position,-ray.ray.direction,true), world.random.Get_Uniform_Number((T)0,(T)1,tid));
            ...
            if(inscattering_amplification>0) for(int light_index=1;light_index<=lights.m;light_index++){
                ...
                if(voxel_object->Use_Precomputed_Light_Data(sample_point,light_index)) {
                    ...
                    for(int sample=1;sample<=sample_array.m;sample++){
                        ...
                        TV light_color=world.Incident_Light(ray_to_light,*lights(light_index),ray_to_light,ray,tid);
...
//#####################################################################
template<class T> VECTOR<T,3> RENDERING_VOXEL_SHADER<T>::
Attenuate_Light(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& object,const RENDERING_LIGHT<T>& light,const TV& light_color,const int tid/*=1*/)
{
    const RENDERING_VOXELS<T>* voxel_object=(const RENDERING_VOXELS<T>*)&object;
    T start_t,end_t;
...
Scatter_Photon_Ray(const RENDERING_OBJECT<T>& object,RENDERING_RAY<T>& ray,TV& photon_power,const typename PHOTON_MAP<T>::PHOTON_MAP_TYPE type,const T fixed_step_size,const int tid/*=1*/)
{
    ...
    if(u>p){// scatter
        ...
        if(world.random.Get_Uniform_Number((T)0,(T)1,tid)<p_s){
            ...
            ray.ray.direction=Inverse_Phase_Direction(ray.ray.direction,tid);
