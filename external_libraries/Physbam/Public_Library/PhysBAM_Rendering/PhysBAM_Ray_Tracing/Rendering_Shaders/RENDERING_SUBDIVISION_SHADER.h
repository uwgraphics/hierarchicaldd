...
public:
    ...
    VECTOR<T,3> Shade_Light_Ray(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& exiting_object,const RENDERING_OBJECT<T>& entering_object,
        const RENDERING_OBJECT<T>& intersection_object,const VECTOR<T,3>& intersection_point,const VECTOR<T,3>& same_side_normal,const RENDERING_LIGHT<T>& light,const RENDERING_RAY<T>& full_ray,const int tid=1) const
    {return shader.Shade_Light_Ray(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,light,full_ray,tid);}
    ...
    {// recast ray from start, using secondary intersection for object and requiring a hit.
        ...
        if(surface->Intersection(copy_ray.ray)){
            ...
            VECTOR<T,3> color=shader.Shade_Surface_Using_Direct_Illumination(copy_ray,*ray.current_object,entering_object,intersection_object,intersection_point,same_side_normal,tid);
