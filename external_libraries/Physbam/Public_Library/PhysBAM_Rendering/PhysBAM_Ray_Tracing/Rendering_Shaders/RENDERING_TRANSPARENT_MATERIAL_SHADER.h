...
public:
    ...
    TV Shade_Surface_Using_Direct_Illumination(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& exiting_object,const RENDERING_OBJECT<T>& entering_object,
        const RENDERING_OBJECT<T>& intersection_object,const TV& intersection_point,const TV& same_side_normal,const int tid=1) const
    ...
    if(use_russian_roulette){
      T current_roll=world.random.Get_Uniform_Number((T)0.0,(T)1.0,tid);
    ...
    TV Shade_Light_Ray(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& exiting_object,const RENDERING_OBJECT<T>& entering_object,
        const RENDERING_OBJECT<T>& intersection_object,const TV& intersection_point,const TV& same_side_normal,const RENDERING_LIGHT<T>& light,const RENDERING_RAY<T>& full_ray,const int tid=1) const
    ...
    return transmission_coefficient*world.Incident_Light(transmitted_ray,light,full_ray,ray,tid);}
    ...
    {T reflection_coefficient,transmission_coefficient;Compute_Coefficients(reflection_coefficient,transmission_coefficient,ray,same_side_normal);
    T xi=world.random.Get_Uniform_Number((T)0,(T)1,tid);
