...
Calculate_Surface_Irradiance(const TV& point,const TV& same_side_normal,RENDER_WORLD<T>& world,const RENDERING_OBJECT<T>& intersection_object,const int tid/*=1*/) 
{
    ...
    for(int light_index=1;light_index<=lights.m;light_index++){
        ...
        for(int sample=1;sample<=sample_array.m;sample++){
            ...
            TV light_color=world.Incident_Light(ray_to_light,*lights(light_index),ray_to_light,dummy_root,tid);
...
//#####################################################################
template<class T> TV RENDERING_BSSRDF_SHADER<T>::
Shade_Surface_Using_Direct_Illumination(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& exiting_object,const RENDERING_OBJECT<T>& entering_object,const RENDERING_OBJECT<T>& intersection_object,const TV& intersection_point,const TV& same_side_normal,,const int tid/*=1*/) const
{
    ...
    TV diffuse_color(color_shader->Shade_Surface_Using_Direct_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,tid));
    ...
    if(ambient_shader) ambient_irradiance=VECTOR<double,3>(ambient_shader->Shade_Surface_Using_Direct_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,tid));
