...
public:
        ...
        const RENDERING_OBJECT<T>& intersection_object,const VECTOR<T,3>& intersection_point,const VECTOR<T,3>& same_side_normal,const int tid=1) const
    {
        ...
        return shader->Shade_Surface_Using_Direct_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,perturbed_normal,tid);
    }
    ...
    {    
        ...
        VECTOR<T,3> tangent,bitangent;intersection_object.Get_Object_Space_Tangent_And_Bitangent(object_space_intersection,object_space_normal,ray.ray.aggregate_id,tangent,bitangent);
        VECTOR<T,3> perturbed_normal=intersection_object.World_Space_Vector(Perturbed_Normal(intersection_object,object_space_intersection,s,t,object_space_normal,tangent,bitangent));
        return shader->Shade_Surface_Using_Approximate_Full_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,perturbed_normal);
    }
        ...
        const RENDERING_OBJECT<T>& intersection_object,const VECTOR<T,3>& intersection_point,const VECTOR<T,3>& same_side_normal) const 
    {
        ...
        VECTOR<T,3> tangent,bitangent;intersection_object.Get_Object_Space_Tangent_And_Bitangent(object_space_intersection,object_space_normal,ray.ray.aggregate_id,tangent,bitangent);
        VECTOR<T,3> perturbed_normal=intersection_object.World_Space_Vector(Perturbed_Normal(intersection_object,object_space_intersection,s,t,object_space_normal,tangent,bitangent));
        return shader->Shade_Surface_Using_Indirect_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,perturbed_normal);
    }
        ...
        const RENDERING_RAY<T>& full_ray,const int tid=1) const
    {
        ...
        VECTOR<T,3> tangent,bitangent;intersection_object.Get_Object_Space_Tangent_And_Bitangent(object_space_intersection,object_space_normal,ray.ray.aggregate_id,tangent,bitangent);
        VECTOR<T,3> perturbed_normal=intersection_object.World_Space_Vector(Perturbed_Normal(intersection_object,object_space_intersection,s,t,object_space_normal,tangent,bitangent));
        return shader->Shade_Light_Ray(ray,exiting_object,entering_object,intersection_object,intersection_point,perturbed_normal,light,full_ray,tid);
    }
        ...
        const typename PHOTON_MAP<T>::PHOTON_MAP_TYPE type,const int diffuse_bounces,const int specular_bounces,const int tid=1) const 
    {
        ...
        VECTOR<T,3> tangent,bitangent;intersection_object.Get_Object_Space_Tangent_And_Bitangent(object_space_intersection,object_space_normal,ray.ray.aggregate_id,tangent,bitangent);
        VECTOR<T,3> perturbed_normal=intersection_object.World_Space_Vector(Perturbed_Normal(intersection_object,object_space_intersection,s,t,object_space_normal,tangent,bitangent));
        return shader->Receive_Photon(ray,exiting_object,entering_object,intersection_object,intersection_point,perturbed_normal,power,type,diffuse_bounces,specular_bounces,tid);
    }
