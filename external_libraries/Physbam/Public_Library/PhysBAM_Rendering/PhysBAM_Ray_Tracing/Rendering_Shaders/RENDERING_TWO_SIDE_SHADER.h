...
public:
    ...
    {return VECTOR<T,3>::Dot_Product(ray.ray.direction,intersection_object.Normal(intersection_point,ray.ray.aggregate_id))>0?
        back_shader.Shade_Surface_Using_Direct_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,tid):
        front_shader.Shade_Surface_Using_Direct_Illumination(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,tid);}
    ...
    {if(VECTOR<T,3>::Dot_Product(ray.ray.direction,intersection_object.Normal(intersection_point,ray.ray.aggregate_id))>0)
        back_shader.Receive_Photon(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,power,type,diffuse_bounces,specular_bounces,tid);
    else front_shader.Receive_Photon(ray,exiting_object,entering_object,intersection_object,intersection_point,same_side_normal,power,type,diffuse_bounces,specular_bounces,tid);}
