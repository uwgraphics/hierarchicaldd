...
public:
    ...
    ADVECTION_WRAPPER_ALE(T_NESTED_ADVECTION& nested_advection_input,RIGID_GRID<T_GRID>& rigid_grid_input,const int number_of_ghost_cells_input=0)
    ...
    {T_GRID cell_center_grid=T_GRID(grid.numbers_of_cells,RANGE<TV>(grid.domain.min_corner+(T).5*grid.dX,grid.domain.max_corner-(T).5*grid.dX));
        T_ARRAYS_VECTOR V(grid.Domain_Indices(number_of_ghost_cells));
