...
{
    T minimum_edge_length_squared=FLT_MAX;
    for(int s=1;s<=mesh.elements.m;s++){
        int i,j;mesh.elements(s).Get(i,j);
        minimum_edge_length_squared=min(minimum_edge_length_squared,(particles.X(i)-particles.X(j)).Magnitude_Squared());}
    return sqrt(minimum_edge_length_squared);
}
//#####################################################################
