...
public:
    ...
    FACTORY_BASE<T_BASE_OBJECT>* factory=0;
    if(registry.name_registry.Contains(name)) factory=registry.name_registry.Get(name);
    else{
        factory=new FACTORY<T_OBJECT,T_BASE_OBJECT>();
        registry.name_registry.Insert(name,factory);}
    if(!extension.empty()){
        if(registry.extension_registry.Contains(extension)) PHYSBAM_ASSERT(factory==registry.extension_registry.Get(extension));
        else registry.extension_registry.Insert(extension,factory);}}
