...
RIGID_GEOMETRY(RIGID_GEOMETRY_COLLECTION<TV>& rigid_geometry_collection_input,bool create_collision_geometry,int index)
    :implicit_object(0),simplicial_object(0),rigid_geometry_collection(rigid_geometry_collection_input),is_static(false),is_deformable(false),bounding_box_up_to_date(false),moving_simplex_hierarchy(0),impulse_accumulator(0)
{
...
{
    if(!is_deformable) return Pointwise_Object_Velocity(Twist(),this->X(),X);
    assert(implicit_object);return implicit_object->Velocity(X);
}
//#####################################################################
