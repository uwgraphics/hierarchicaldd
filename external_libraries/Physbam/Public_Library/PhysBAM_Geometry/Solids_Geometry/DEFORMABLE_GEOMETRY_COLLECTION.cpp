...
DEFORMABLE_GEOMETRY_COLLECTION(GEOMETRY_PARTICLES<TV>& particles_input,bool own_structures_input/*=true*/)
    :particles(particles_input),own_structures(own_structures_input)
{
...
{
    if(own_structures) structures.Delete_Pointers_And_Clean_Memory();
}
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
//#####################################################################
// Function Read
...
Read(const STREAM_TYPE stream_type,const std::string& prefix,const std::string& static_prefix,const int frame,const int static_frame,const bool include_static_variables,const std::string& postfix)
{
    Read_Dynamic_Variables(stream_type,prefix,frame,postfix);
    if(include_static_variables) Read_Static_Variables(stream_type,static_prefix,static_frame,postfix);
}
//#####################################################################
// Function Write
//#####################################################################
template<class TV> void DEFORMABLE_GEOMETRY_COLLECTION<TV>::
Write(const STREAM_TYPE stream_type,const std::string& prefix,const std::string& static_prefix,const int frame,const int static_frame,const bool include_static_variables,const std::string& postfix) const
{
    Write_Dynamic_Variables(stream_type,prefix,frame,postfix);
    if(include_static_variables) Write_Static_Variables(stream_type,static_prefix,static_frame,postfix);
}
//#####################################################################
// Function Read_Static_Variables
//#####################################################################
template<class TV> void DEFORMABLE_GEOMETRY_COLLECTION<TV>::
Read_Static_Variables(const STREAM_TYPE stream_type,const std::string& prefix,const int frame,const std::string& postfix)
{
    std::string f=frame==-1?"common":FILE_UTILITIES::Number_To_String(frame);
    std::istream* input_raw=FILE_UTILITIES::Safe_Open_Input(prefix+"/"+f+"/deformable_object_structures"+postfix);
...
//#####################################################################
template<class TV> void DEFORMABLE_GEOMETRY_COLLECTION<TV>::
Read_Dynamic_Variables(const STREAM_TYPE stream_type,const std::string& prefix,const int frame,const std::string& postfix)
{
    FILE_UTILITIES::Read_From_File(stream_type,prefix+"/"+FILE_UTILITIES::Number_To_String(frame)+"/deformable_object_particles"+postfix,particles);
...
//#####################################################################
template<class TV> void DEFORMABLE_GEOMETRY_COLLECTION<TV>::
Write_Static_Variables(const STREAM_TYPE stream_type,const std::string& prefix,const int frame,const std::string& postfix) const
{
    std::string f=frame==-1?"common":FILE_UTILITIES::Number_To_String(frame);
    std::ostream* output_raw=FILE_UTILITIES::Safe_Open_Output(prefix+"/"+f+"/deformable_object_structures"+postfix);
...
//#####################################################################
template<class TV> void DEFORMABLE_GEOMETRY_COLLECTION<TV>::
Write_Dynamic_Variables(const STREAM_TYPE stream_type,const std::string& prefix,const int frame,const std::string& postfix) const
{
    FILE_UTILITIES::Write_To_File(stream_type,prefix+"/"+FILE_UTILITIES::Number_To_String(frame)+"/deformable_object_particles"+postfix,particles);
}
#endif
//#####################################################################
