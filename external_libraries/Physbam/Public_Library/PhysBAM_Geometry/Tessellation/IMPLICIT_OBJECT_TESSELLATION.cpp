...
template<class T> TRIANGULATED_SURFACE<T>* Generate_Triangles_Helper(const IMPLICIT_OBJECT_COMBINED<VECTOR<T,3> >* implicit)
{
    ...
    GRID<TV> grid(implicit->Minimum_Cell_Size(),implicit->box);
    ARRAY<T,TV_INT> phi(grid.Domain_Indices());
    LEVELSET_IMPLICIT_OBJECT<TV> levelset(grid,phi);
    for(typename GRID<TV>::CELL_ITERATOR iterator(grid);iterator.Valid();iterator.Next()){phi(iterator.Cell_Index())=implicit->Extended_Phi(grid.X(iterator.Cell_Index()));}
    TRIANGULATED_SURFACE<T>* triangles=Generate_Triangles(levelset);
    return triangles;
}
