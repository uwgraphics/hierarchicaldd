...
public:
    using BASE::array_collection;using BASE::X;using BASE::V;using BASE::id;
    using BASE::Store_Velocity;using BASE::Store_Id;

    ARRAY_VIEW<T> radius;
    ARRAY_VIEW<T> mass;
    ARRAY_VIEW<T> pressure;
    ARRAY_VIEW<T> radial_V;
    ARRAY_VIEW<T> cumulative;
    ARRAY_VIEW<TV> relative_V;
    ARRAY_VIEW<ARRAY<PAIR<TV_INT,T> > > weights;
    BUBBLE_PARTICLES():radius(0,0),mass(0,0),pressure(0,0),radial_V(0,0),cumulative(0,0),relative_V(0,0),weights(0,0){
        ...
        array_collection->Add_Array(ATTRIBUTE_ID_CUMULATIVE,&cumulative);
        array_collection->Add_Array(ATTRIBUTE_ID_RELATIVE_VELOCITY,&relative_V);
        array_collection->Add_Array(ATTRIBUTE_ID_WEIGHTS,&weights);
        Store_Velocity();Store_Id();}
