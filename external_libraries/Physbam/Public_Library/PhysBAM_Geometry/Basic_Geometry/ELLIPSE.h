...
public:
    ...
    {
        TV temp = orientation.Inverse_Rotate(location-center);
        TV radii_v;radii_v(1)=radii(1,1);radii_v(2)=radii(2,2);
        for(int i=1;i<=2;i++)temp(i)/=sqr(radii_v(i));
        return orientation.Rotate(temp.Normalized());
    }
