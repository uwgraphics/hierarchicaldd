...
//#####################################################################
template<class T_GRID> void PCG_SPARSE_BUBBLE_MPI<T_GRID>::
Parallel_Solve(SPARSE_MATRIX_FLAT_NXN<T>& A,VECTOR_ND<T>& x,VECTOR_ND<T>& b,const T tolerance,const bool recompute_preconditioner)
{
    ...
    int global_n=(int)Global_Sum_Bubbles((T)interior_n);
    ...
    if(pcg.enforce_compatibility && pcg.remove_null_space_solution_component) x_interior-=(T)(Global_Sum_Bubbles((T)(x_interior.Sum_Double_Precision())/global_n));

    // find initial residual, r=b-Ax - reusing b for the residual
    Fill_Ghost_Cells(x);
    A.Times(x,temp);b_interior-=temp_interior;
    if(pcg.enforce_compatibility) b_interior-=(T)(Global_Sum_Bubbles((T)(b_interior.Sum_Double_Precision())/global_n));
    ...
    for(int iteration=1;;iteration++){
        ...
        if(pcg.enforce_compatibility) z_interior-=(T)(Global_Sum_Bubbles((T)(z_interior.Sum_Double_Precision())/global_n));

        // update search direction
        rho_old=rho;rho=Global_Sum_Bubbles((T)VECTOR_ND<T>::Dot_Product_Double_Precision(z_interior,b_interior));
        ...
        Fill_Ghost_Cells(p);
        A.Times(p,temp);
        T alpha=(T)(rho/Global_Sum_Bubbles((T)VECTOR_ND<T>::Dot_Product_Double_Precision(p_interior,temp_interior)));
...
//#####################################################################
template class PCG_SPARSE_BUBBLE_MPI<GRID<VECTOR<float,1> > >;
template class PCG_SPARSE_BUBBLE_MPI<GRID<VECTOR<float,2> > >;
template class PCG_SPARSE_BUBBLE_MPI<GRID<VECTOR<float,3> > >;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class PCG_SPARSE_BUBBLE_MPI<GRID<VECTOR<double,1> > >;
template class PCG_SPARSE_BUBBLE_MPI<GRID<VECTOR<double,2> > >;
template class PCG_SPARSE_BUBBLE_MPI<GRID<VECTOR<double,3> > >;
#endif
#endif
