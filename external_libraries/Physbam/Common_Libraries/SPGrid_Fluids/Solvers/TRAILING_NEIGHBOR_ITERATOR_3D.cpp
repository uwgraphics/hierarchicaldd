...
// Constructor
//#####################################################################
template<class T_STRUCT,class T> TRAILING_NEIGHBOR_ITERATOR<T_STRUCT,T,3>::
...
//#####################################################################
template<class T_STRUCT,class T> void TRAILING_NEIGHBOR_ITERATOR<T_STRUCT,T,3>::
Enumerate_Neighbors()
{
    ...
    if(flags & SPGrid_Face_Minus_Z_Active)
    {
        ...
        {
            T* data_ptr = &hierarchy.Array(cid.level, data_field)(neighbor_offset);
            T* coef_ptr = &hierarchy.Array(cid.level, Uz_field)(coef_offset);
            neighbors.Append(NEIGHBOR(neighbor_level, neighbor_offset, data_ptr, coef_ptr));
        }
    }
...
//#####################################################################
template class TRAILING_NEIGHBOR_ITERATOR<FLUIDS_SIMULATION_DATA<float>,float,3>;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class TRAILING_NEIGHBOR_ITERATOR<FLUIDS_SIMULATION_DATA<double>,double,3>;
#endif
