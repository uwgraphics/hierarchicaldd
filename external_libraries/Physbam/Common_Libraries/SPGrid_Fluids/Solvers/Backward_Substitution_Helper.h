...
//#####################################################################
#ifndef __Backward_Substitution_Helper__
#define __Backward_Substitution_Helper__
#include <algorithm>
...
namespace SPGrid{
template<class T,int log2_struct, int d> class Backward_Substitution_Helper;

template<class T,int log2_struct>
class Backward_Substitution_Helper<T,log2_struct,2>
{
    enum{d=2};
    typedef SPGrid_Mask<log2_struct, NextLogTwo<sizeof(T)>::value,d> T_MASK;

    T* const x;                   // Solution of backward substitution
    T* const diag;                // Diagonal of factorized matrix
    const T* const Lx;
    const T* const Ly;
...
public:
    explicit Backward_Substitution_Helper(T* const x_input,T* const diag_input,const T* const Lx_input,const T* const Ly_input,
        const unsigned* const mask_input,const unsigned long* const b_input,const int size_input)
        :x(x_input),diag(diag_input),Lx(Lx_input),Ly(Ly_input),mask(mask_input),b(b_input),size(size_input)
    {}
...
template<class T,int log2_struct>
class Backward_Substitution_Helper<T,log2_struct,3>
{
    enum{d=3};
    typedef SPGrid_Mask<log2_struct, NextLogTwo<sizeof(T)>::value,d> T_MASK;
...
public:
    explicit Backward_Substitution_Helper(T* const x_input,T* const diag_input,const T* const Lx_input,const T* const Ly_input,const T* const Lz_input,
        const unsigned* const mask_input,const unsigned long* const b_input,const int size_input)
        :x(x_input),diag(diag_input),Lx(Lx_input),Ly(Ly_input),Lz(Lz_input),mask(mask_input),b(b_input),size(size_input)
    {}
