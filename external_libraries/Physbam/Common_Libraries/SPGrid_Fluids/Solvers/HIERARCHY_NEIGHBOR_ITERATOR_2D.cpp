...
// Constructor
//#####################################################################
template<class T_STRUCT, class T> HIERARCHY_NEIGHBOR_ITERATOR<T_STRUCT,T,2>::
...
//#####################################################################
template<class T_STRUCT,class T> void HIERARCHY_NEIGHBOR_ITERATOR<T_STRUCT,T,2>::
Enumerate_Neighbors()
{
    ...
    while(!cell_stack.empty())
    {
        ...
        if(hierarchy.Set(cid.level).Is_Set(cid.offset,mask))
        {
            ...
            switch(mask)
            {
                case SPGrid_Face_Minus_X_Active:
                    offset = Flag_array_mask::template Packed_Offset<-1, 0>(cid.offset);
                ...
                case SPGrid_Face_Minus_Y_Active:
                    offset = Flag_array_mask::template Packed_Offset< 0,-1>(cid.offset);
                    data_ptr = &hierarchy.Array(cid.level, data_field)(offset);
        ...
        else
        {
            ...
            switch(mask)
            {
                case SPGrid_Face_Minus_X_Active:
                    offsetA = base_offset;
                    offsetB = Flag_array_mask::template Packed_Offset<0,1>(base_offset);
                    break;
                case SPGrid_Face_Plus_X_Active:
                    offsetA = Flag_array_mask::template Packed_Offset<1,0>(base_offset);
                    offsetB = Flag_array_mask::template Packed_Offset<1,1>(base_offset);
                    break;
                case SPGrid_Face_Minus_Y_Active:
                    offsetA = base_offset;
                    offsetB = Flag_array_mask::template Packed_Offset<1,0>(base_offset);
                    break;
                case SPGrid_Face_Plus_Y_Active:
                    offsetA = Flag_array_mask::template Packed_Offset<0,1>(base_offset);
                    offsetB = Flag_array_mask::template Packed_Offset<1,1>(base_offset);
                    break;
                default:
                    std::cout<<"Reached default case of cell-splitter"<<std::endl;
...
//#####################################################################
template class HIERARCHY_NEIGHBOR_ITERATOR<FLUIDS_SIMULATION_DATA<float>,float,2>;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class HIERARCHY_NEIGHBOR_ITERATOR<FLUIDS_SIMULATION_DATA<double>,double,2>;
#endif
