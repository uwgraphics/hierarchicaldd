...
//#####################################################################
#ifndef __Blocked_Set_Helper__
#define __Blocked_Set_Helper__
namespace SPGrid{

template<class T,int elements_per_block>
class Blocked_Set_Helper
{
    T* const x;         // output stream
    const T c;  // input stream
...
public:
    explicit Blocked_Set_Helper(T* const x_input,const T c_input, const unsigned* const flags_input, const unsigned long* const b_input, const int size_input)
        :x(x_input),c(c_input),flags(flags_input),b(b_input),size(size_input)
    {
    }
