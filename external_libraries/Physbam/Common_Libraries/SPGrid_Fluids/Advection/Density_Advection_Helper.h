...
//#####################################################################
#ifndef __Density_Advection_Helper_h__
#define __Density_Advection_Helper_h__
...
class Density_Advection_Helper
{
    ...
    enum{nodes_per_cell=GRID_TOPOLOGY_HELPER<typename Data_array_type::MASK>::nodes_per_cell};
    ...
    T T_STRUCT::* density_field;
    T T_STRUCT::* node_density_field;
    ...
    const T dt;
    const unsigned density_advect_mask;
    const int level;
    const RIGID_GEOMETRY_COLLECTION<TV>* const rigid_geometry_collection;

public:

    Density_Advection_Helper(const T_HIERARCHY& hierarchy_input,T T_STRUCT::* density_field_input,T T_STRUCT::* node_density_field_input,
        T T_STRUCT::* result_field_input,T_FLAGS T_STRUCT::* flags_field_input,const VECTOR<void*,d>& node_velocity_ptrs_input,
        unsigned long (&nodes_of_cell_offsets_input)[nodes_per_cell],const TV intra_cell_dX_input,const T dt_input,
        const unsigned density_advect_mask_input,const int level_input,const RIGID_GEOMETRY_COLLECTION<TV>* const rigid_geometry_collection_input)
        :hierarchy(hierarchy_input),density_field(density_field_input),node_density_field(node_density_field_input),result_field(result_field_input),
         flags_field(flags_field_input),node_velocity_ptrs(node_velocity_ptrs_input),nodes_of_cell_offsets(nodes_of_cell_offsets_input),
         intra_cell_dX(intra_cell_dX_input),dt(dt_input),density_advect_mask(density_advect_mask_input),level(level_input),
         rigid_geometry_collection(rigid_geometry_collection_input)
    {}

    Density_Advection_Helper(SPGrid_Allocator<T_STRUCT,d>& allocator,const std::pair<const unsigned long*,unsigned>& blocks,
        const T_HIERARCHY& hierarchy_input,T T_STRUCT::* density_field_input,T T_STRUCT::* node_density_field_input,
        T T_STRUCT::* result_field_input,T_FLAGS T_STRUCT::* flags_field_input,const VECTOR<void*,d>& node_velocity_ptrs_input,
        unsigned long (&nodes_of_cell_offsets_input)[nodes_per_cell],const TV intra_cell_dX_input,const T dt_input,
        const unsigned density_advect_mask_input,const int level_input,const RIGID_GEOMETRY_COLLECTION<TV>* const rigid_geometry_collection_input)
        :hierarchy(hierarchy_input),density_field(density_field_input),node_density_field(node_density_field_input),result_field(result_field_input),
