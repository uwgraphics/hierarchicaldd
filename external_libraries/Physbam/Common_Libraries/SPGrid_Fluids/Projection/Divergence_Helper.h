...
namespace SPGrid{
template<class T,int log2_struct, int d> class Divergence_Helper;

template<class T,int log2_struct>
class Divergence_Helper<T,log2_struct,2>
{
    enum{d=2};
    typedef SPGrid_Mask<log2_struct, NextLogTwo<sizeof(T)>::value,d> T_MASK;
    T* const cell_data;  // output stream
    const T* const x_faces;         // input stream
    const T* const y_faces;         // input stream
...
public:
    explicit Divergence_Helper(
        T* const cell_input,
        const T* const x_input,
        const T* const y_input,
        const unsigned* const mask_input,
        const unsigned long* const b_input,
        const int size_input,
        const double scale_input)
        : 
...
template<class T,int log2_struct>
class Divergence_Helper<T,log2_struct,3>
{
    enum{d=3};
    typedef SPGrid_Mask<log2_struct, NextLogTwo<sizeof(T)>::value,d> T_MASK;
    T* const cell_data;  // output stream
    const T* const x_faces;         // input stream
    const T* const y_faces;         // input stream
    const T* const z_faces;         // input stream
    const unsigned* const mask;
    const unsigned long* const b;   // block offset stream
    const int size;     // number of blocks to process
    const double scale;
