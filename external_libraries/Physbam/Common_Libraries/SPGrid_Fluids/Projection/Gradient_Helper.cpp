...
#else
        if ( mask_value & (SPGrid_Cell_Type_Interior|SPGrid_Cell_Type_Ghost|SPGrid_Cell_Type_Dirichlet)) {
            
            const double cell_value = (double)(cell_data_in[cur_index]);

            if (mask_value & SPGrid_Face_Minus_X_Scaled)
                x_face_output[cur_index] = scale_nonuniform * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i-1][j])));
            else if (mask_value & SPGrid_Face_Type_X_Active)
                x_face_output[cur_index] = scale_uniform * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i-1][j])));
            
            if (mask_value & SPGrid_Face_Minus_Y_Scaled)
                y_face_output[cur_index] = scale_nonuniform * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i][j-1])));
            else if (mask_value & SPGrid_Face_Type_Y_Active)
                y_face_output[cur_index] = scale_uniform * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i][j-1])));
        }                
#endif
    ...
    for(int index=index_start;index<=index_end;index++) {
      ...
      for(int k=zmin;k<=zmax;k++)
      {
        ...
        if ( mask_value & (SPGrid_Cell_Type_Interior|SPGrid_Cell_Type_Ghost|SPGrid_Cell_Type_Dirichlet)) {
            ...
            if (mask_value & SPGrid_Face_Minus_X_Scaled)
                x_face_output[cur_index] = scale_nonuniform * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i-1][j][k])));
            else if (mask_value & SPGrid_Face_Type_X_Active)
                x_face_output[cur_index] = scale_uniform    * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i-1][j][k])));
            
            if (mask_value & SPGrid_Face_Minus_Y_Scaled)
                y_face_output[cur_index] = scale_nonuniform * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i][j-1][k])));
            else if (mask_value & SPGrid_Face_Type_Y_Active)
                y_face_output[cur_index] = scale_uniform    * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i][j-1][k])));
            
            if (mask_value & SPGrid_Face_Minus_Z_Scaled)
                z_face_output[cur_index] = scale_nonuniform * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i][j][k-1])));
            else if (mask_value & SPGrid_Face_Type_Z_Active)
                z_face_output[cur_index] = scale_uniform    * (cell_value - (double)(*reinterpret_cast<T*>(cell_base_addr + o_grid[i][j][k-1])));
...
template class Gradient_Helper<double,5,2>;
template class Gradient_Helper<double,6,2>;

template class Gradient_Helper<double,5,3>;
template class Gradient_Helper<double,6,3>;
#endif
