...
//#####################################################################
// Class OPENGL_SPGRID_3D
//##################################################################### 
#ifndef __OPENGL_SPGRID_3D__
#define __OPENGL_SPGRID_3D__
...
template<class T>
class OPENGL_SPGRID_3D : public OPENGL_OBJECT
{
public:
...
public:
    OPENGL_SPGRID_3D(GRID<TV> &fine_mac_grid_input,bool& draw_separate_levels_input,int& level_counter_input,bool& draw_nodal_flags_input,int& nodal_flag_counter_input,const std::string& filename_input,const OPENGL_COLOR &color_input=OPENGL_COLOR::White(),const int frame_input=0)
    ...
    void Draw_Cell(const RANGE<TV>& domain,const TV& corner,const TV& dX) const PHYSBAM_OVERRIDE;
    ...
    DEFINE_CALLBACK_CREATOR(OPENGL_SPGRID_3D, Toggle_Draw_Separate_Levels);
    DEFINE_CALLBACK_CREATOR(OPENGL_SPGRID_3D, Toggle_Draw_Boundary_Conditions);
    DEFINE_CALLBACK_CREATOR(OPENGL_SPGRID_3D, Toggle_Draw_Nodal_Flags);
};

template<class T>
class OPENGL_SELECTION_SPGRID_CELL_3D : public OPENGL_SELECTION
{
private:
    typedef VECTOR<T,3> TV;typedef VECTOR<int,3> TV_INT;
public:
    int level;
    TV_INT index;
    OPENGL_SELECTION_SPGRID_CELL_3D(OPENGL_OBJECT *object,const int level=1,const TV_INT& index=TV_INT()) 
        : OPENGL_SELECTION(OPENGL_SELECTION::GRID_CELL_3D, object),level(level),index(index) {}
