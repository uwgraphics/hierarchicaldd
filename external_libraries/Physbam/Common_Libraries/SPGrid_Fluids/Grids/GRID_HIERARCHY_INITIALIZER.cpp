...
//#####################################################################
// Function Flag_Edge_Ghost_Cells
//#####################################################################
template<class T_STRUCT,class T,int d> void GRID_HIERARCHY_INITIALIZER<T_STRUCT,T,d>::
Flag_Edge_Ghost_Cells(Hierarchy_type& hierarchy)
{
    LOG::SCOPE scope("GRID_HIERARCHY_INITIALIZER::Flag_Edge_Ghost_Cells");

    typedef Parity_Helper<Flag_array_mask,d> Parity;
    //LOG::cout<<"Tagging ghost cells"<<std::endl;
    static const int number_of_edge_neighbors=(d==2)?4:12;
    unsigned long edge_neighbor_shifts[number_of_edge_neighbors];
    const int levels = hierarchy.Levels();

    for(int v_axis=0,edge_neighbor=0;v_axis<d;v_axis++){
        for(int w_axis=v_axis+1;w_axis<d;++w_axis){
            for(int v_shift=-1;v_shift<=1;v_shift+=2){
                for(int w_shift=-1;w_shift<=1;w_shift+=2){
                    std_array<int,d> shift;
                    shift.data[v_axis]=v_shift;
                    shift.data[w_axis]=w_shift;
                    std::cout<<shift<<std::endl;
                    edge_neighbor_shifts[edge_neighbor++]=Flag_array_mask::Linear_Offset(shift);}}}}
    ...
    for(int level=1;level<levels;level++){
        ...
        for(SPGrid_Set_Iterator<Flag_array_type> iterator(hierarchy.Set(level));iterator.Valid();iterator.Next())
        {
            if(iterator.Data() & SPGrid_Cell_Type_Interior)
            {
                for(int edge_neighbor=0;edge_neighbor<number_of_edge_neighbors;edge_neighbor++)
                {
                    unsigned long neighbor_offset=iterator.Offset(edge_neighbor_shifts[edge_neighbor]);
                    unsigned long cur_offset=neighbor_offset;
                    // Now search at this and coarser resolutions for parent
                    int parent_level = 0;
                    for (int cur_level=level;cur_level<=levels;cur_level++)
...
template class GRID_HIERARCHY_INITIALIZER<FLUIDS_SIMULATION_DATA<double>,double,2>;
template class GRID_HIERARCHY_INITIALIZER<FLUIDS_SIMULATION_DATA<double>,double,3>;
#endif
