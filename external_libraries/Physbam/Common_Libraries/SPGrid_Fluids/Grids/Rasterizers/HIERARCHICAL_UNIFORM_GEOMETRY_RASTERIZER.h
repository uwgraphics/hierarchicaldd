...
//#####################################################################
// Class HIERARCHICAL_UNIFORM_GEOMETRY_RASTERIZER
//#####################################################################
#ifndef __HIERARCHICAL_UNIFORM_GEOMETRY_RASTERIZER__
#define __HIERARCHICAL_UNIFORM_GEOMETRY_RASTERIZER__
...
class HIERARCHICAL_UNIFORM_GEOMETRY_RASTERIZER:public HIERARCHICAL_RASTERIZER<T_STRUCT,T,d>
{
...
public:
    ...
    HIERARCHICAL_UNIFORM_GEOMETRY_RASTERIZER(GRID_HIERARCHY<T_STRUCT,T,d>& hierarchy_input,const RIGID_GEOMETRY<TV>* const rigid_geometry_input)
    ...
    bool Consume(const T_CELL& cell)
    {
        ...
        for(int i=0;i<GRID<TV>::number_of_nodes_per_cell;++i) phi[i]=rigid_geometry->Implicit_Geometry_Extended_Value(hierarchy.Grid(level).Node(nodes[i]));
        bool outside=true;
        ...
        if(level==1 && rigid_geometry->Implicit_Geometry_Extended_Value(center)<=(T)2.*hierarchy.Grid(level).dX.Min())
