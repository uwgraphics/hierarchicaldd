...
#include <SPGrid_Fluids/Rendering/OpenGL/OPENGL_COMPONENT_SPGRID_SCALAR_FIELD_3D.h>
#include <SPGrid_Fluids/Rendering/OpenGL/OPENGL_COMPONENT_SPGRID_MAC_VELOCITY_FIELD_3D.h>
...
private:
    ...
    OPENGL_COMPONENT_SPGRID_3D<T>* spgrid_component;
    OPENGL_COMPONENT_OCTREE<T>* octree_component;
    OPENGL_COMPONENT_PARTICLES_3D<T,VORTEX_PARTICLES<TV> >* vortex_particles_component;
    ...
    GRID<TV> grid,mac_grid,regular_grid,coarse_grid,coarse_mac_grid,coarse_regular_grid,fine_grid,octree_fine_grid;
    bool *draw_separate_levels;
    int *level_counter;
    bool *draw_nodal_flags;
    int *nodal_flag_counter;
    bool has_valid_grid,has_valid_rle_grid,has_valid_octree_grid,has_valid_coarse_grid,has_valid_fine_grid,has_valid_octree_fine_grid;
...
#endif
    ,grid_component(0),coarse_grid_component(0),spgrid_component(0),octree_component(0),vortex_particles_component(0)
#ifndef COMPILE_WITHOUT_DYADIC_SUPPORT
    ,octree_grid_component(0)
#endif
    ,draw_separate_levels(new bool(false)),level_counter(new int(0)),draw_nodal_flags(new bool(false)),nodal_flag_counter(new int(0))
{
}
...
~VISUALIZATION()
{
    ...
    if(spgrid_component) delete spgrid_component;
    if(octree_component) delete octree_component;
#ifndef COMPILE_WITHOUT_RLE_SUPPORT
...
Read_Grid()
{
    ...
    if(FILE_UTILITIES::File_Exists(filename)){
        ...
        has_valid_grid=true;}
    else if(FILE_UTILITIES::File_Exists(STRING_UTILITIES::string_sprintf("%s/common/fine_grid",basedir.c_str()))){
        filename=STRING_UTILITIES::string_sprintf("%s/common/fine_grid",basedir.c_str());
        LOG::cout<<"Reading fine_grid from '"<<filename<<"'..."<<std::flush;
        FILE_UTILITIES::Read_From_File<RW>(filename,fine_grid);
        filename=basedir+"/%d/";
        spgrid_component=new OPENGL_COMPONENT_SPGRID_3D<T>(fine_grid,filename,*draw_separate_levels,*level_counter,*draw_nodal_flags,*nodal_flag_counter,start_frame);
        has_valid_fine_grid=true;}
    else if(FILE_UTILITIES::File_Exists(basedir+"/common/octree_fine_grid")){
        filename=basedir+"/common/octree_fine_grid";
        std::cout<<"Reading octree fine grid from '"<<filename<<"'..."<<std::endl;
        FILE_UTILITIES::Read_From_File<RW>(filename,octree_fine_grid);
        filename=basedir+"%d/octree";
        octree_component=new OPENGL_COMPONENT_OCTREE<T>(filename,start_frame);
        has_valid_octree_fine_grid=true;}
...
Initialize_Components_And_Key_Bindings()
{
    ...
    if(has_valid_fine_grid){
        OPENGL_UNIFORM_SLICE* slice=new OPENGL_UNIFORM_SLICE(opengl_world);slice->Initialize(fine_grid);
        slice_manager.slice=slice;
        std::cout<<"Using SPGrid slice"<<std::endl;}
    if(has_valid_octree_fine_grid){
        OPENGL_UNIFORM_SLICE* slice=new OPENGL_UNIFORM_SLICE(opengl_world);slice->Initialize(octree_fine_grid);
        slice_manager.slice=slice;
        std::cout<<"Using octree fine grid slice"<<std::endl;}
    ...
    // Voronoi Diagram
    filename=basedir+"/%d/voronoi_diagram";
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_VORONOI_DIAGRAM_3D<T>* voronoi_diagram_component=new OPENGL_COMPONENT_VORONOI_DIAGRAM_3D<T>(filename,start_frame);
        Add_Component(voronoi_diagram_component,"Voronoi",'I',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);}
...
//        levelset_component->Set_Surface_Material(OPENGL_MATERIAL::Plastic(OPENGL_COLOR((T).6,(T).65,1)),OPENGL_MATERIAL::Plastic(OPENGL_COLOR((T).6,(T).65,1)));
    ...
    filename=basedir+"/%d/octree_phi";
    if(has_valid_octree_fine_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_OCTREE_SCALAR_FIELD<T>* levelset_component=new OPENGL_COMPONENT_OCTREE_SCALAR_FIELD<T>(octree_component->opengl_octree,filename,OPENGL_COLOR_RAMP<T>::Levelset_Color_Constant_Ramp(OPENGL_COLOR::Blue(),OPENGL_COLOR::Red(.5)));
        opengl_world.Set_Key_Binding_Category("Phi");
        Add_Component(levelset_component,"Phi",'l',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::START_HIDDEN);
        opengl_world.Append_Bind_Key('L',levelset_component->Toggle_Color_Map_CB());
        slice_manager.Add_Object(levelset_component);}
    filename=basedir+"/%d/octree_pressure";
    if(has_valid_octree_fine_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        OPENGL_COMPONENT_OCTREE_SCALAR_FIELD<T>* pressure_component=new OPENGL_COMPONENT_OCTREE_SCALAR_FIELD<T>(octree_component->opengl_octree,filename,OPENGL_COLOR_RAMP<T>::Levelset_Color_Constant_Ramp(OPENGL_COLOR::Blue(),OPENGL_COLOR::Red(.5)));
        ...
        opengl_world.Append_Bind_Key('&',pressure_component->Toggle_Color_Map_CB());
        slice_manager.Add_Object(pressure_component);}

    opengl_world.Set_Key_Binding_Category("Octree");
    filename=basedir+"/%d/octree";
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        Add_Component(octree_component,"Octree",'O',BASIC_VISUALIZATION::SELECTABLE|BASIC_VISUALIZATION::START_HIDDEN);
        slice_manager.Add_Object(octree_component);}
    ...
    filename=basedir+"/%d/spgrid_density";
    if(has_valid_fine_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        T T_STRUCT::* density_channel=&T_STRUCT::ch3;
        OPENGL_COMPONENT_SPGRID_SCALAR_FIELD_3D<T>* density_component=new OPENGL_COMPONENT_SPGRID_SCALAR_FIELD_3D<T>(fine_grid,spgrid_component->opengl_spgrid,filename,density_channel,OPENGL_COLOR_RAMP<T>::Two_Color_Ramp(0,1,OPENGL_COLOR::Gray(0,0),OPENGL_COLOR::Gray(1,1)),*draw_separate_levels,*level_counter);
        opengl_world.Set_Key_Binding_Category("Density");
        Add_Component(density_component,"Density",'d',BASIC_VISUALIZATION::OWNED);
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F8),density_component->Toggle_Draw_Mode_CB());
        opengl_world.Append_Bind_Key('D',density_component->Toggle_Color_Map_CB());
        slice_manager.Add_Object(density_component);}
    filename=basedir+"/%d/spgrid_u";
    if(has_valid_fine_grid && FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        T T_STRUCT::* u_channel=&T_STRUCT::ch0;T T_STRUCT::* v_channel=&T_STRUCT::ch1;T T_STRUCT::* w_channel=&T_STRUCT::ch2;
        std::string v_filename=basedir+"/%d/spgrid_v";std::string w_filename=basedir+"/%d/spgrid_w";
        OPENGL_COMPONENT_SPGRID_MAC_VELOCITY_FIELD_3D<T>* mac_velocity_component=new OPENGL_COMPONENT_SPGRID_MAC_VELOCITY_FIELD_3D<T>(fine_grid,spgrid_component->opengl_spgrid,filename,v_filename,w_filename,u_channel,v_channel,w_channel,*draw_separate_levels,*level_counter);
        ...
        opengl_world.Append_Bind_Key('V',mac_velocity_component->Toggle_Draw_CB());
    ...
    if(has_valid_fine_grid){
        Add_Component(spgrid_component,"SPGrid",'6',BASIC_VISUALIZATION::START_HIDDEN|BASIC_VISUALIZATION::SELECTABLE);
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F2),spgrid_component->opengl_spgrid.Toggle_Draw_Nodal_Flags_CB());
        opengl_world.Append_Bind_Key(OPENGL_KEY(OPENGL_KEY::F1),spgrid_component->opengl_spgrid.Toggle_Draw_Boundary_Conditions_CB());
        opengl_world.Append_Bind_Key('^',spgrid_component->opengl_spgrid.Toggle_Draw_Separate_Levels_CB());
        slice_manager.Add_Object(spgrid_component);}
    ...
    filename=basedir+"/%d/vortex_particles";
    if(FILE_UTILITIES::Frame_File_Exists(filename,start_frame)){
        vortex_particles_component=new OPENGL_COMPONENT_PARTICLES_3D<T,VORTEX_PARTICLES<TV> >(filename,"",false);
        vortex_particles_component->particles->array_collection->template Add_Array<int>(ATTRIBUTE_ID_ID);
        vortex_particles_component->opengl_points->color=OPENGL_COLOR::Yellow();
        vortex_particles_component->opengl_points->point_size=2;
        Add_Component(vortex_particles_component,"Vortex particles",'O',BASIC_VISUALIZATION::OWNED|BASIC_VISUALIZATION::SELECTABLE|BASIC_VISUALIZATION::START_HIDDEN);
        if(slice_manager.slice) slice_manager.Add_Object(vortex_particles_component);}
...
Pre_Frame_Extra()
{
    ...
    if(spgrid_component) spgrid_component->opengl_spgrid.Set_Frame(frame);
}
